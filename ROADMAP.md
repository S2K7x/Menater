...
| Non-JSON refusals on the `notify` bot transport | The remaining sibling of the row above, left alone deliberately. `makeNotify`'s `slack-bot` branch still calls `res.json()` with no guard, so an HTML error page from a gateway in front of Slack throws a `SyntaxError` rather than naming the status. Slack's own API does answer JSON on its errors — including on `ok: false` — so this is reachable only through an intermediary, and it is the one branch of the three whose success is READ from the body. Worth a guard, not worth risking the transport whose test file exists because it lied about success once already |
