...
| ~~Raw transport errors inside the engine~~ ✅ | ~~The `io.ts` nodes let a bare `TypeError: "fetch failed"` become a step error~~ — done. `reach()` wraps the five call sites of `http` / `notify` / `llm` and names the destination and the cause through `describeFetchError`, and the destination is a WORD or a host, never the URL, because a webhook URL is itself the credential. The same pass found the other half of the defect and fixed it: `llm` never looked at the status, so a 401 on a wrong key was reported as *"The model answered with no content"* and a body-less 429 as *"Unexpected end of JSON input"*. `nodes/io-failures.test.ts` pins both, including through the real engine to the run journal |
| ~~A body refused for its SIZE, reported as three other things~~ ✅ | ~~`readBody`'s 256 kB cap threw a plain `Error`, and every caller read it as something it was not~~ — done. The cap worked; the answer did not. `PUT /api/ingestion/policy` swallowed the refusal into `null`, which `normalizePolicyInput` reads as *change nothing*, and answered **200 with the current policy**: a refused save reported as a save. `POST /api/findings/promote` and the two ingestion endpoints named fields nobody had read (*`scan_run_id`: a non-empty string is required*). The other fifteen routes got the last net's **500** carrying `Corps de requete trop volumineux.` — a server-fault code, in French, in an English-only product. Now one class: `BodyTooLarge` carries the cap, `app.ts` answers **413** worded through `humanBytes` (`Request body over 256 kB…`) from the `api` catalogue, and the three tolerant callers take `readBodyOrNull`, which still answers `null` for a body that could not be READ. `server/body-limit.test.ts`, nine tests, eight of them red before the change and each of four mutations red after — including the plausible wrong fix of lowering the cap until nothing gets through. Driven over a real socket under `node --experimental-strip-types` as well as through the handler |
| ~~A named 5xx whose sentence the browser threw away~~ ✅ | ~~`lib/api.ts` replaced the body of any 502/503/504 with the generic « the console server is not answering »~~ — done. The rule was written for the Vite dev proxy, which answers **502 with an empty `text/plain` body** (measured on Vite 8.2.1 against a dead upstream): nothing named a reason there, and the generic sentence is right. The console answers 503 too and never for that reason — no database configured, so the rule store cannot be read and the engine is not mounted; or a configured database refusing, where the sentence lost is the one `describePgError` exists to recover. The Rules tab, the dry run, an approval being answered and the Health tab's injection button all told the operator to restart a server that had just answered them, while *Settings → Database* sat in a string that never left the process. `call()` now reads the body before deciding and keeps a named `error`; a blank one is not a reason. `POST /api/simulate` answers **200** in its own `{ ok, response }` envelope instead, like the catch two lines below it. `src/lib/api-named-failures.test.ts`, ten tests, six red before the change and each of four mutations red after — including deleting the special case altogether, which the four controls catch. It drives the REAL handler through the REAL client, so the assertion is the sentence that reaches the screen. **Not fixed, same trap**: a forwarded 401 still sends an operator to the login screen, and a 400's explanation is still read from `body.error` alone |
| ~~The snapshot is serialised and compressed once per REQUEST~~ ✅ | ~~`json()` re-ran `JSON.stringify` and `gzipSync` over an object the cache was handing back unchanged~~ — done. Re-measured on a synthetic window of the documented size (563,315 bytes raw, 48,396 gzipped, 11.6:1 — the real pipeline's figures were 563.9 kB / 45.3 kB): **6.12 ms of blocked event loop per request**, 2.49 ms of it `JSON.stringify` and 3.35 ms `gzipSync`, on the single thread that also serves the ingestion webhook. `json()` now encodes a body ONCE PER OBJECT IDENTITY, in a `WeakMap` so the bytes die with the body: the repeat answer costs **0.1-0.6 us**, measured through the real function. Identity is the whole safety argument — a body that changed is necessarily a different object, so there is no invalidation anybody can forget, the same reasoning as the service inventory's index. The one thing it forbids is answering twice with an object MUTATED in between, and no route does that. The saving needed a second half: the route composed `{ ...snap, refresh_seconds }`, a NEW object per request, so the cadence travels on the snapshot now (`withRefreshRate`, and `refreshSeconds` joined `cacheKey`). **It scales with READERS, not with polls** — one tab at the default 20 s cadence saves nothing, since each poll gets its own rebuild; ten tabs at the 5 s minimum share one encoding for the cache's fifteen seconds, which is ~29 requests out of 30. `server/respond-encoding.test.ts`, ten tests, four red before the change, driving the REAL handler for the route half; four mutations, each red |
| ~~`trace.chains[].payload` is shipped in full and read as a boolean~~ ✅ | ~~The browser's only use of it is `if (!chain.payload)`~~ — done. The five fields of the alert as the webhook received it travelled to every open tab; `POST /api/replay` re-reads them SERVER-side out of the same snapshot, because the browser posts an `alert_id` and nothing else. They are held against the trace they belong to, in a `WeakMap` — the identity argument the response memo and the inventory index already use, so a rebuilt snapshot cannot be answered out of the previous one's alerts — and `replayPayload` is the only way back to them. The chain carries `replayable: boolean`, which is what was read. **The « negligible once gzipped » in this row was an assumption, and it was wrong.** Re-measured through the real pipeline and the real `buildCases`: at the default window (24 alerts, 120 runs) **162,113 → 144,696 bytes raw (−10.7%) and 15,952 → 13,608 gzipped (−14.7%)**; at the 500-run ceiling (100 alerts) 672,640 → 599,854 raw and **61,374 → 54,979 gzipped (−6,395 B, −10.4%)**. The compressed saving is proportionally LARGER than the raw one because gzip cannot absorb the duplicate: the case's `raw_log` and the chain's copy of it sit **85,644 B apart** at the default window and 351,637 B at the ceiling, far outside DEFLATE's 32,768-byte window. With the short logs this row was first measured on, still −947 B and −7.4% gzipped. **And the field was unusable as what it was for.** It stored the observables through the incident card's display fallback — an em dash — and those five fields are re-injected at the entry point, whose validator rightly refuses a destination that is present and not an address. Measured by driving `POST /api/replay` through the real handler over the real pipeline: `400 — invalid_dest_ip: not an IPv4/IPv6 literal`, i.e. **the replay button refused every alert with no destination address**, which § 2 bis says is most of them. A string shaped for a screen is not an input — the `shortLabel` trap of J0.1, in a second place. `server/engine/trace-payload.test.ts` (7 tests) reads the answer off the SERIALISED trace, and `server/replay-route.test.ts` (5 tests) drives the route end to end and asserts on what the ENGINE received, out of the run journal, rather than on the sentence the route printed. Four mutations, each red |
| ~~A refusal explained under a key the client did not read~~ ✅ | ~~`lib/api.ts` read a failed request's explanation from `body.error` alone~~ — done, and it closes the 4xx half of the poisoned-status-codes trap. This server names a reason under three keys, and two of them were discarded: `problems` (one `{field, detail}` per thing wrong with a tuning rule, from `POST /api/rules` and `PUT /api/rules/:id`) and `detail` (from `POST /api/approvals/:token/resume` and the database probe). Measured through the real handler and the real client: five precise sentences about a rule, and the approval failure naming both the cause and that *the alert will be escalated*, all reached the operator as *"The console server answered 400 with no explanation."* `RulesPage` has rendered `err.problems` line by line since it was written and had never received one — dead code that looks load-bearing. `namedReason` is one reader for both branches: `error` wins when several keys are present, a blank or non-string reason is not a reason (an `error` of `"  "` had been painting a blank banner, and `/api/mcp`'s object-valued `error` was one call away from `[object Object]`), and the structured list travels on the `ApiError` so the banner receives it. `src/lib/api-named-failures.test.ts` gains seven tests, all red before the change, plus `src/components/rules-refusal.test.tsx`, which mounts the editor behind the real client and the real handler and reads the four lines off the banner. Four mutations, each red |
| ~~Non-JSON refusals on the `notify` bot transport~~ ✅ | ~~`makeNotify`'s `slack-bot` branch calls `res.json()` with no guard~~ — done. The branch reads the body as TEXT and parses it itself, and `ok` being a boolean is what tells the Slack API's answer from an intermediary's: an HTML error page, a captive portal's sign-in page or a body-less 5xx now names the status and quotes what came back, instead of surfacing `Unexpected token '<'` or `Unexpected end of JSON input` (both measured on Node 22). The same pass found the half that was not commissioned: with the parse failing first, nothing had ever reached the `if (!body.ok)` below it with `ok` absent — so a **captive portal answering 200** would have been called a refusal BY Slack, sending an operator to check a channel name and a bot scope over a network problem. `Slack refused: …` is now reachable only from the `ok: false` envelope it describes. Six tests in `nodes/io-failures.test.ts`, three of them red before the change. **Completed while unblocking the PR**: the guard's own `catch(() => '')` conflated a body that could not be READ with an empty one — `res.text()` rejects after the status line is in (`TypeError: terminated`, cause `other side closed`, probed on Node 22.22.2) — and the `slack-webhook` branch beside it caught nothing at all. Three states, three sentences; three more tests, two of them red |
| ~~A human's approval, read back as a refusal~~ ✅ | ~~`cases.ts` read the settled approval under three names `interpretApproval` does not write, and the request under two `buildApprovalRequest` does not write~~ — done. The transform returns `{ outcome, proposed_action, approval }`; the reader looked for a top-level `approved`, so it was false for every answer a human ever gave and the card said *« Declined by unknown »*, *« No reason given »*, on a case whose action had just been executed. `approval_triggers` read as `triggers` meant the *« why you are being asked »* block never rendered; the deadline was on the request under no name at all (`ttl_minutes` beside it is the ISOLATION TTL), so the card printed a hardcoded 30 — the default value, hence right until somebody changed it. The cost is `human_disagreement_rate_pct`, which § Measurement calls the rate that gates leaving shadow mode: it read **100** whenever anyone approved anything, measured. `buildApprovalRequest` now carries `timeout_minutes` (it already quoted it to Slack and to nobody else), the reader reads every field where the transform writes it, and `interpret` is read LAST because it is the node that decides. Six tests in `engine/pipeline-to-case.test.ts` driving the real pipeline through a live approval — the file's first non-shadow run, which is why the defect had survived it — plus three in `case-view.test.tsx` for a banner branch that had never been reachable. Four mutations, each red |
| ~~An approval answered in the console is interpreted as SILENCE~~ ✅ | ~~`routes/approvals.ts` posted `{ approved, human_reasoning, approver: {…} }` while `interpretApproval` reads `payload.decision`~~ — done. **Three parties spoke about one approval and two of them already agreed**: the browser client posts `{ decision, approver, reason }` (`src/lib/api.ts`) and the transform reads exactly those three names; the route in the middle was the only one inventing words, and nothing on the far side read them. So `p.decision` was always `undefined` and the transform's own rule — anything that is not exactly *approve* or *reject* is silence — turned a human's yes into `timeout_escalated`. Measured through the REAL route over the REAL pipeline: the approved action was **not executed** (`https://edr.test/isolate` never dialled) and the audit record — append-only and immutable by design — read `routing_outcome: "rejected"`, `action_taken: "none"`, `"reason": "Action refused by a human."` about a yes. **The cost to the measurement is worse than this row claimed**: it said the disagreement rate read 100, which was true before the reader fix; measured now, `computeMetrics` counts only cases whose outcome is `approved` or `rejected`, so every settled approval reading `timeout_escalated` left the denominator at 0 and `human_disagreement_rate_pct` read **null** — not a wrong number, no number at all, on the one figure § Measurement calls the gate for leaving shadow mode. The route speaks the pipeline's vocabulary now; the decision stays a WORD and never a boolean, because `approve`, `reject` and *nobody answered* are three facts and a boolean has no room for the third; and a word the route does not recognise is refused with a named **400** instead of being forwarded, since `resolveWait` is irreversible and forwarding it would spend the token and file a silence that never happened. The vocabulary question this row raised is answered the other way round from the obvious: the route sends a CLAIM and the guardrail mints the RECORD, so `identity_source: 'console_self_declared'` — the value `CLAUDE.md` and `dashboard/README.md` already document, produced by nothing until now — moved INTO `interpretApproval`, where no payload can author it. `server/approval-route.test.ts`, nine tests, all nine red before the change, five mutations of which four are caught (the fifth, a route forwarding the whole body, is behaviour-preserving precisely because the guardrail mints) |
| ~~The approve and reject buttons are disabled on every real case~~ ✅ | ~~`CaseView` refuses to submit without `approval.execution_id`, and `cases.ts` never wrote that field~~ — done, and it is the other half of the path the row above opened: with the payload fixed, the console still could not answer an approval at all. Measured by driving the real pipeline to a live wait and reading the case the console would render: `state: 'awaiting_approval'`, `approval` present, `approval.execution_id: undefined`, so `Boolean(execId)` was `false` and BOTH buttons rendered `disabled` — with nothing saying why, since the other term of the same `&&` is the operator's own name, so a greyed-out button is indistinguishable from *you have not typed your identifier yet*. Only `demo.ts` set it, which is why the screen looked right in demonstration mode. **The identifier was the same disagreement one field over**: `src/lib/api.ts` names the parameter `executionId` and `CaseView` passes a RUN id, while the route read the path segment as a wait TOKEN — engine tokens are `crypto.randomUUID()`, so even a populated field would have answered 404. **The run id wins, and the reason is what the token IS.** A wait token resolves an approval exactly once and irreversibly: it is a verb, not a name. Carrying it to the browser would have put it on the incident card, which `get_alert` answers with `sections: ['approval']` to a model whose context also holds attacker-composed log text, and to every MCP client holding a read-only bearer token — the catalogue this product keeps closed and verbless precisely so an injected instruction finds nothing to reach for. Run ids already travel there, on every stage line, so the card gained no capability it did not have. `RunStore.openWaitOfRun` does the resolving on the server (`WHERE run_id = $1 AND resumed_at IS NULL`, covered by `soc_run_wait_run_idx`), `Engine.resumeRun` is the one caller, and `POST /api/approvals/:runId/resume` is what the route now reads. A consequence worth naming: a wait somebody has already settled is no longer open, so the second of two simultaneous presses is answered **404** — *« that approval is not open any more; the first answer stands »*, the sentence the catalogue already held — where it used to be told **200, decision relayed** about a decision that was not the one recorded. `server/approval-route.test.ts` gains four tests and reads its identifier off `buildCases` rather than naming a token, so **10 of its 13 are red** before the change; `store-contract.test.ts` gains four, run against both implementations (Postgres when `MENATER_TEST_PG` is set); `case-view.test.tsx` gains two that claim the browser side of the link and pass before and after, on purpose. Five mutations, each caught by the assertion that should catch it |
| ~~Nothing calls `sweepExpiredWaits()`~~ ✅ | ~~Grepped exhaustively: the only callers were `engine.test.ts` and `integration.test.ts`~~ — done for the approval half. There was no scheduler in the process at all: `setInterval` appeared in `ingest/poller.ts` for polling and in `auth.ts` for sessions, and nowhere else. So the approval TIMEOUT never fired: the request posted to a human promised *« no answer within 30 minutes and the alert is escalated »*, and the run simply stayed `waiting` for ever. *Silence is never consent* held — nothing was executed — but the escalation meant to replace it never happened, and because `awaiting` short-circuits the verdict ladder in `cases.ts` the chain never became `stalled` either, so the Tracking tab did not show it. **Everything around the sweep was already built and already correct**: the `timeout` port is wired to `escalate-timeout` in `04-Action-Routing`, `cases.ts` reads that node and sets `outcome: 'timeout_escalated'`, and `sql/10-engine.sql` even carries the partial index the sweep query needs (`soc_run_wait_pending_idx ON soc_run_wait (deadline) WHERE resumed_at IS NULL`) — a schema built for a periodic sweep that was never scheduled. `server/engine/scheduler.ts` is that timer, started in `api.ts` beside `syncPoller()` and stopped beside `stopPoller()`. It reads `getEngine()` on EVERY tick, so a database configured from the Settings tab starts honouring deadlines with no restart; it never rejects out of the timer (an uncaught rejection there terminates a Node 22 process); it runs one sweep at a time; and it says a failure ONCE plus its recovery, because a line a minute repeating one sentence is the permanent alarm this console refuses everywhere else. The same pass closed a defect the wiring MADE live: the sweep loop had no fault isolation, so a human answering one approval in the instant between `expiredWaits` listing it and `resolveWait` settling it threw out of the whole sweep — measured on the in-memory store — and every OTHER expired approval behind it was left waiting, in silence. Isolation is per wait, and the classification is read from the store (the wait having gained a `resumedAt`) rather than matched on the error text |
| ~~The store contract's Postgres half had never run, and it was red~~ ✅ | ~~`store-contract.test.ts` runs the SAME suite against `MemoryRunStore` and `PgRunStore`, and skips the second unless `MENATER_TEST_PG` names a database~~ — done. It had never been set, so the file written to stop the two implementations diverging **had never once compared them**, and they had diverged: `MemoryRunStore.resolveWait` threw `jeton d'attente inconnu` where `PgRunStore` throws `unknown wait token`. That sentence is not internal — `routes/approvals.ts` puts `err.message` straight into `approvalFailed`, so the console answered an operator in a different language depending on which store was mounted, in an English-only product. Found by standing a real Postgres 16.13 up in the nightly container and running the skipped half: **1 failed, 45 passed** on unmodified `main`. The memory store says the English sentence now and the contract asserts it. **Two things the run settled that no PR could assert on its own**: the four `openWaitOfRun` contract tests added with the approval fix PASS against real Postgres, so that SQL (`WHERE run_id = $1 AND resumed_at IS NULL ORDER BY created_at LIMIT 1`) is verified rather than reviewed; and the premise several nights inherited — *the pg-store half cannot be verified, this environment has no database* — is simply false. `initdb` refuses to run as root, so the cluster is created under the `postgres` user and inside `/var/lib/postgresql` (it cannot traverse the scratchpad path); `sql/*.sql` apply cleanly except `01-role-and-dedup.sql`, whose `:'app_password'` is a psql variable `docker/init-db.sh` supplies. **What is NOT fixed is that CI still does not run it**, so the next drift is as invisible as this one was |
| **An executed action is sealed into the audit chain as `action_taken: "none"`** | Found while verifying the row above, and reproduced on unmodified `main` at 85a77b6, so it is neither new nor caused by it. Driving an alert to a live approval and answering *approve*: the containment really runs — `https://edr.test/isolate` is dialled and the case reads `executed: true` — while the audit record 04 composes reads **`action_taken: "none"`, `routing_outcome: "unknown"`**, and the incident card's `action_taken` is `null`. So the append-only row that is this product's only tamper-evidence does not name the action it is evidence of, and the card cannot say WHICH action ran. On the card side the immediate cause is that `cases.ts` reads `action` / `executed_action` off the `execute` step, which is an `http` node whose output is `{ ok, status, body }` and carries neither; the audit side is its own question, since `buildAuditRecord` is what writes those two words. Same family as the three approval rows above — the record disagreeing with what happened — and **the existing test passes over it because it asserts `routing_outcome` is NOT `rejected` and NOT `timeout_escalated`**, which `"unknown"` satisfies: a negative assertion cannot notice a non-answer. Not fixed here because it changes what is sealed into an immutable chain, which is a subject of its own |
| **Nothing calls `engine.resume()` outside the tests** | The other half of the row above, deliberately left open — wiring it would look fixed and would not be. `resume()` claims each unfinished run with `claimRun(id, owner)`, whose SQL is `WHERE id = $1 AND (owner IS NULL OR owner = $2)`, and `drive()` releases the claim on every terminal path — so a process that DIED mid-run leaves `soc_run.owner` set to `console-<its pid>` for ever, and nothing releases it. A restarted console presents a different owner and the claim is refused and the run is skipped, silently — which is exactly the crash recovery this would be wired to perform. Whether the restarted process happens to present the SAME owner is an accident of the runtime, not a guarantee: a fresh container namespace may hand node the same pid, `npm run serve` on a host will not, and a recovery that works only when two pids collide is not a recovery. The engine's own test suite says so in as many words (*« ne reprend pas une exécution déjà prise par un autre processus »*), and its recovery tests only pass because `MemoryRunStore.simulateCrash()` clears the locks — a thing Postgres cannot do by itself. So the missing piece is not a `setInterval`, it is an answer to *when is a claim stale*: a `claimed_at` older than some age, a process identity that survives a restart, or an explicit release. That is a decision, and a wrong one replays a `write` step. An interrupted run therefore still shows `running` in `soc_run` — which the Tracking tab DOES surface, unlike the silent approval wait |
| ~~A caller refused at the ingestion door still spent the console's most expensive read~~ ✅ | ~~`routes/ingest.ts` called `invalidate()` unconditionally, after `handleAlert` had returned~~ — done. `/api/ingest/:source` and `/api/webhook/soc/alert` sit OUTSIDE the console's access lock on purpose — an appliance emitting an alert has no browser session — and through a tunnel they are the part of this product that faces the Internet. `handleAlert` refuses there in four ways before the engine is reached (mode off, no secret configured, wrong token, no engine), and every one of them still threw the snapshot cache away: `invalidate()` drops `cache` AND `inFlight`, so the next reader waits on a full walk of the run journal and concurrent readers stop sharing one — the two properties `snapshot.ts`'s own header exists to provide, calling the rebuild *"the most expensive thing the console does"*. **Measured over a real socket against the real service under `node --experimental-strip-types`, console lock ON, no credential of any kind sent: 30 unauthenticated POSTs answered `401` caused 30 of 30 snapshot rebuilds; after the fix, 0 of 30.** Once per request, with no rate limit, on the single thread that also serves the alerts — and nothing shows it: the attacker sees a 401, the operator sees a console that got slow. The fix is not a status-code test, because the status cannot answer the question in either direction: three of those refusals are `503` and the PIPELINE answers a 503 of its own when deduplication is unavailable, while a `400` for a missing field is an alert that DID run and is visible in the Tracking tab. `WebhookResult` carries `ran`, set where the engine is actually started, and the route reads it. `server/refused-ingest.test.ts` asserts the EFFECT rather than spying on the call — it compares two snapshots by identity, so a fix that merely moved `invalidate()` would not pass — eight tests, six red before the change, plus a mirror that fails if the fix simply deletes the call; `webhook.test.ts` gains three that pin the flag at its source. Three mutations, each red |
| ~~The MCP cap counted one of the two doors to the same work~~ ✅ | ~~`rateLimited` was called from `case 'tools/call'` and from nowhere else~~ — done. Its own comment says why it exists — *"these tools each read the pipeline snapshot, and an agent in a loop is exactly the client that will call them a thousand times without noticing"* — and `resources/read` runs those same getters: `readResource` calls `runTool` for eight of the nine `menater://` URIs, and ITS comment insists on it, *"every branch goes through `runTool`, NOT around it"*. Two comments, each correct, describing one uncounted path. **Measured on the real endpoint: 400 `tools/call` → 120 served, 280 refused; 400 `resources/read` → 400 served, 0 refused.** And it is reachable in BULK, because a JSON-RPC batch is one request dispatched with `Promise.all` over whatever fits in the 256 kB body — the `Promise.all`-over-the-whole-window trap this codebase has already paid for once: **a single 179 kB batch of 2,000 `resources/read` served all 2,000 and blocked the event loop for 123 ms**, on the thread that also answers the ingestion webhook. After: 120 served in 17 ms, worst stall 2 ms. The cap is charged BEFORE the URI is resolved, which is the one place it differs from `tools/call` — copying `readResource`'s vocabulary up to spare a typo one unit of a 120-a-minute budget would be a second list to keep in step. What is deliberately NOT metered is claimed by tests that pass before and after: the catalogue listings, which are constants, and `completion/complete`, which is one call per KEYSTROKE. Six tests in `assistant/mcp.test.ts`, four red before the change; three mutations, each red — including the one the first draft of the batch test missed, a budget that resets per request |
| ~~`normalize()` breaks its "never throws" contract on deeply nested JSON, unauthenticated~~ ✅ | ~~`routes/ingest.ts` calls `normalize(mapping, await readBodyOrNull(req))` **before** `handleAlert` looks at the shared secret, and `normalize` does `JSON.stringify(v)` on a mapped field with no depth floor~~ — done. `JSON.stringify` is recursive in V8 and `JSON.parse` is not, so the parser hands the serialiser structures it cannot survive: measured on Node 22.22.2, **parse is still fine at 200 000 levels and stringify gives up at 4 165** — and that 4 165 is not a constant either, it is whatever stack is left at the moment of the call, which the same probe under `--stack-size=2000` answers **8 500** for. The reproduction stands: console lock ON, **no `x-soc-token` at all**, a 117 kB body of `{"raw_log":[[[…60 000 deep…]]]}` inside the 256 kB cap because that cap is on BYTES → **`500 {"error":"Maximum call stack size exceeded"}`**, a server-fault code for a sender fault plus V8's internal message forwarded to an unauthenticated caller. **The sweep that wrote this row found one door; there were two, and the second was worse.** `pollSource()` is the other caller of `normalize`, and there the `RangeError` escaped the function entirely — so `fail()`, which records the error, backs the source off and puts it on the Ingestion tab, never ran. It escaped `tick()` too, which the timer calls as `void this.tick()`: **an unhandled rejection from a timer terminates a Node 22 process** (measured), so one over-deep item from a polled source takes the whole console down, on every interval. `MAX_PAYLOAD_DEPTH = 64` and `PayloadTooDeep` in `normalize.ts`, in the shape `BodyTooLarge` already has; the route answers a named **400** and the poller counts the item `unusable`, keeping the rest of the batch. Three decisions are in the PR and worth carrying: the floor is on the **whole payload** and not on the mapped fields, because `extensions` keeps every unmapped key whole and the run journal, the model prompt and the assistant's fence all serialise what arrives; the cap is a **policy number chosen against real data** (the nine injection scenarios are 2 deep, a Wazuh alert with MITRE arrays is 5) rather than a measured maximum that moves with the machine; and the check **refuses on the way down**, because measure-then-compare overflows on exactly the payload it exists to refuse. `server/payload-depth.test.ts`, 13 tests, 8 of them red before the change, and six mutations tried |
| ~~A body that could not be PARSED, read as an empty one~~ ✅ | ~~`readBody` mapped a `JSON.parse` failure to `{}`, under a header vouching that this held "because EVERY route validates what it received"~~ — done, and it is the shape half of the door whose SIZE half was closed two rows up. The claim was never measured and is false in both directions. Measured against the real handler: a truncated body made `PUT /api/ingestion/policy`, `PUT /api/settings`, `PUT /api/credentials` and `PUT /api/workflows/variables` answer **200 with the settled value** — a save that never happened, reported as a save, on the four routes where an operator changes things; the two ingestion endpoints handed `{}` to the pipeline, so an appliance posting form-encoded data is told its alert is missing `alert_id`, a verdict about a body nobody read; and `POST /api/auth/login` answered *"Wrong password."* and **spent one of the eight attempts**, so a client with a broken serialiser locks the operator out. A second shape fell out of the same read: a body that PARSES and is not an object (`null`, a number) reached `body.probeWebhook` and `body.scenario`, and `POST /api/diagnostics` and `POST /api/simulate` answered **500 carrying V8's message**. `readBody` now answers three facts rather than one — nothing arrived is `{}`, bytes that do not parse and bytes that parse to no object are `MalformedBody`, which carries which — `app.ts` words both as a **400** from the catalogue, and `readBodyOrNull` lets them travel like the size refusal, so `null` means only *the stream failed*. `/api/mcp` catches it and answers inside JSON-RPC (**-32700** for the parse failure, **-32600** for valid JSON that is not a request object), which also revives its own `PARSE_ERROR` branch: written for this case, it was unreachable, because `{}` is an object. `server/malformed-body.test.ts`, 18 tests, 9 mutations |
| ~~The database test button answers about a port the console would never dial~~ ✅ | ~~`POST /api/settings/test/database` does `Number(body.port ?? d.port)` and hands it straight to `tcpProbe`~~ — done. `saveConfig` ten lines away already clamps the same value into `[1, 65535]`, so the rule existed — on the screen an operator uses when things ALREADY work. The diagnostic, pressed precisely when they do not, had none, and answered two different wrong things. Measured through the real handler, then over real sockets under `node --experimental-strip-types`: clearing the port field sends `0` (`Number('') === 0` in the `type="number"` onChange) and the button answered **200** *« Connection refused: nothing is listening on this port »* — a verdict about a machine for a value that named no port, and Node's own error for port 0 does not even carry one (`connect ECONNREFUSED 127.0.0.1`, measured), because the OS never dialled it. `70000`, `-1`, `5432.5` and a non-numeric string made `createConnection` throw `RangeError ERR_SOCKET_BAD_PORT` **synchronously inside `tcpProbe`'s promise executor**, which nothing catches, so the route fell to the last net in `app.ts`: **500** carrying Node's own *« Port should be >= 0 and < 65536 »*, logged with a stack under `[menater] uncaught error`. Both are now one named **400** at the route, and NOT a clamp: probing 65535 because somebody typed 70000 answers a question nobody asked. The network question keeps its 200 — `ok: true` with the caveat when something listens, `ok: false` when nothing does — because "is something listening" and "that is not a port" are different questions with different answers. The guard is at the CALLER and deliberately not inside `tcpProbe`, which could only answer `ok: false` there, i.e. the network sentence again. `server/database-probe.test.ts` (8 tests) and one more in `api-named-failures.test.ts`; nine mutations checked |
| ~~VulnPipe's lockfile described the console, and pinned three published advisories~~ ✅ | ~~`VulnPipe/package-lock.json`'s root `packages[""]` block carried the CONSOLE's devDependency list~~ — done. Ten packages this half declares nowhere (`@testing-library/react`, `react`, `react-dom`, `vite`, `jsdom`, `concurrently`, `@vitejs/plugin-react`, `@types/react`, `@types/react-dom`, `@testing-library/user-event`) against the three it does. **It was invisible by construction**: `npm ci` reconciles against `package.json`, so the right tree was installed and the suite, the typecheck and CI were green over it — while the lockfile, which CI runs `npm ci` precisely to trust, made its claim about the wrong package. The cost was the transitive tree frozen around a resolution nobody could refresh: `npm update fast-uri hono qs --package-lock-only` pruned **595 lines before changing one version**, so closing a published advisory looked like a rewrite nobody would review. Behind it, **five advisories, three of them in production dependencies**, all arriving through the single direct dependency `@modelcontextprotocol/sdk@1.30.0`: `fast-uri` 3.1.5 → **3.1.8** (four advisories, CVSS 7.5 — SSRF via malformed IPv6 normalization and via repeated hostname percent-decoding, host confusion ×2), `hono` 4.13.2 → **4.13.8** (three moderate), `qs` 6.15.3 → **6.16.0** (two moderate), plus dev-only `vitest` 4.1.10 → **4.1.11** (`@vitest/mocker` path traversal, GHSA-82fw-gwwq-j7x9 — the console already ran 4.1.11). `npm audit`: **5 → 0**, both halves. **Reachability was measured rather than assumed**, because it decides the urgency this row claims: an instrument reading Node's CJS require cache shows `fast-uri` loading **3 modules on both SDK entry points VulnPipe imports** (`server/mcp.js`, `client/index.js`, through `ajv`), while `hono`, `express` and `qs` load **zero** on every one of them — VulnPipe speaks stdio and never imports the HTTP transport. The control that proves the instrument: `server/streamableHttp.js`, which VulnPipe does not import, does load `hono`. Two findings worth keeping. The obvious remedy was wrong: **`@modelcontextprotocol/sdk@1.30.0` is the latest version**, so no manifest change closes anything — all five fixes sit inside caret ranges the tree already declares. And **npm 10.9.7 cannot resolve `vitest@4.1.11` in this half**, crashing in arborist's peer-set walk (`Cannot read properties of null (reading 'edgesOut')`) where the console's tree resolves it cleanly; npm 12.0.2 resolves it, the lockfile stays `lockfileVersion` 3 with no npm-12-only fields, and `npm ci` under npm 10.9.7 installs it, is byte-stable across runs and reports 0 vulnerabilities. `src/lockfile.test.ts` pins the rule on **both** halves from one place — six assertions, one red before the change, three mutations each caught by exactly one |
| ~~The console's own interface travelled uncompressed~~ ✅ | ~~`respond.ts` gzips any JSON answer over 4 kB, and `static.ts` ten lines away served a 474 kB JavaScript bundle in the clear~~ — done. Measured on a real socket against this repository's build, counting bytes as they travelled rather than trusting Vite's report: a cold load of the console moved **629,637 bytes**, and moves **175,581** now — **−72.1 %**, 3.6 times the bytes, on the one deployment mode CLAUDE.md calls normal, where nothing sits in front of this process to compress instead. Nothing fails, which is why it survived. **Streamed, not `gzipSync`, and that choice is measured**: `gzipSync` on the bundle is 12.4 ms of CPU and **15.3 ms of BLOCKED EVENT LOOP**, `createGzip()` does the same work on libuv's threadpool for a **2.1 ms** lag — this is the thread that also answers the ingestion webhook. **And no memo**, where `respond.ts` has one: that memo answers one object asked for many times a second, whereas every asset here carries a fingerprint and a year of `immutable`, so a browser fetches it once per deployment and there is no repetition to save. The floor is `GZIP_MIN_BYTES`, imported rather than restated — `index.html` is 2,945 bytes and stays in the clear, and a woff2 is a compressed container that gzipping would cost CPU for nothing, so the list says what to COMPRESS rather than what to skip. `Vary: Accept-Encoding` on exactly the answers that depend on the header, because a cache that hands the gzipped variant to an `identity` client would be wrong for a year. Five tests on a REAL socket — the fake response cannot vouch for a body, since `pipe()` on something that is not a `Writable` moves nothing — two red before, three claiming the boundaries and passing before and after |
| ~~Both MCP servers pretty-print every answer they serialise~~ ✅ | ~~`assistant/mcp.ts` wrote `JSON.stringify(result, null, 2)` into `content[0].text` and into every `resources/read`, and `VulnPipe/src/mcp-server/server.ts` did the same in `okResult` / `errorResult`~~ — done. The panel half of the same catalogue has always serialised compactly (`chat.ts`'s `capResult`, `providers.ts`), so this is the mirror of a rule that was only ever held on one side. The text block is what the other side's MODEL reads — `structuredContent` beside it is for the client's code — and two-space indentation adds a line break and an indent run per key. Measured through the real endpoint on the sample window: **24,219 → 19,473 B over the fourteen tools (−19.6 %)**, 26,120 → 21,177 over the nine resources (−18.9 %), and a whole `get_alert` answer 12,132 → 11,033 B. VulnPipe: **10,684 → 7,940 B (−25.7 %)**, −29.6 % on a context bundle at the 24 kB budget, where the only client reads `structuredContent` and never opens `content` at all — so those bytes were serialised, pushed over stdio and discarded. The duplicate block STAYS: the spec asks a tool returning structured content to also return the serialised form, and the survey figure this was carried under (*« 11,191 B where 5,123 B would do »*) was pricing its deletion, not the indentation. Four tests, two red before on each half; five mutations, each red |
| ~~The console scrolled sideways on a phone, and nothing measured it~~ ✅ | ~~`site/check.cjs` sweeps the marketing site for horizontal overflow at 320, 390, 768 and 1400 px, and says in its own header that *« a page that scrolls sideways on a phone hides its own right-hand edge »*. The CONSOLE — the product — had no such check~~ — the defect is fixed, the check is not written. Measured in Chromium on the built bundle: at 320 px the Ingestion tab's document was **330 px**, Health **529** and Settings → MCP **546**; at 375 px, Health and MCP the same. WCAG 2.2 § 1.4.10 (Reflow, AA) names 320 px, which is also 375 px at 125 % zoom. Three causes, all now fixed and tested (`narrow-viewport.test.tsx`): a 51-character connection string flooring a grid track through `min-width: auto`, an `inline-flex` segmented picker shared by six screens that never wrapped, and a 43-character file path in prose. **What stays open is the instrument**: the suite cannot see this — jsdom computes no layout, so `scrollWidth` is 0 — and the console's equivalent of `check.cjs` needs Playwright, which `dashboard/` does not depend on and a night should not add on its own. Until then the rule is three assertions over `styles.css`, not a measurement |
