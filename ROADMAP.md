...
| ~~The approve and reject buttons are disabled on every real case~~ ✅ | ~~`CaseView` needs `approval.execution_id` and `cases.ts` never wrote it~~ — done, and both halves of the path were broken. The console could not answer an approval at all: `canSubmit = approver.trim().length > 0 && Boolean(execId) && !busy`, so with the field absent BOTH controls rendered `disabled` on every real case, saying nothing — the other term of that `&&` is the operator's own name, so a greyed-out button reads as *you have not typed your identifier yet*. Only `demo.ts` set it, which is why the screen looked right in demonstration mode and only there. **The second half was worse than this row claimed.** Posting the identifier the card DOES hold — the `04-Action-Routing` run — answered **404 « That approval is not open any more — it was already answered, or it timed out. The first answer stands; nothing was lost. »** over a wait that was wide open and a run sitting in `waiting`: a confident sentence about a state nobody is in, on the one control carrying this product's central guarantee. **The decision this row left open is answered by measuring what is already published.** The wait token is published NOWHERE — `resumeUrl` builds `/?run=<run id>`, the snapshot carries run ids on every stage of every case, and nothing ever writes a token outside the database, so its only holder is the store. The run is therefore the handle the product already uses for an approval, and accepting it grants no reach a signed-in tab did not have; carrying the token into the snapshot would have created a capability that does not exist today. So `RunStore.openWaitOfRun(runId)` (memory + Postgres, on the `soc_run_wait_run_idx` index the schema has carried since day one for a lookup nobody had written), `Engine.resumeWait` tries the token first and the run second — both are UUIDs, so there is nothing to disambiguate — and it resolves `wait.token`, never its argument. `openWaitOfRun` returns only an OPEN wait, which is what makes the 404 above TRUE when it is finally shown: a second press finds no question, and the first answer stands. **And the environment blocked nothing**: Postgres 16.13 is installable in the nightly container, so `store-contract.test.ts` — written to be run against both implementations and skipped for want of `MENATER_TEST_PG` — ran for real. It immediately reported the drift it exists to catch: `MemoryRunStore` threw `jeton d'attente inconnu` where `PgRunStore` throws `unknown wait token`, and that sentence reaches an operator through `approvalFailed(err.message)`. Six contract tests (both stores), five route tests driving the REAL route over the REAL pipeline and asserting on what the ENGINE concluded, two `CaseView` tests claiming both sides of the `&&`; four mutations, each caught. Verified end to end on a real Postgres: the approved containment dialled, the run `done`, the wait closed, a second press refused |
| **An executed action is sealed into the audit chain as `action_taken: "none"`** | Found while verifying the row above on a real database, and reproduced on unmodified `main`, so it is neither new nor mine. Driving an alert to a live approval and answering *approve*: the containment really runs — `https://edr.test/isolate` is dialled and the case reads `executed: true` — while the audit record composed by 04 reads **`action_taken: "none"`, `routing_outcome: "unknown"`**, and the incident card's `action_taken` is `null`. So the append-only row that is this product's only tamper-evidence does not name the action it is evidence of, and the card cannot say WHICH action ran. The immediate cause is that `cases.ts` reads `action` / `executed_action` off the `execute` step, which is an `http` node whose output is the HTTP response (`{ ok, status, body }`) and carries neither; the audit side is its own question, since `buildAuditRecord` is what writes `"none"` and `"unknown"`. Same family as the two approval rows above — the record disagreeing with what happened — and the existing test passes over it because it asserts `routing_outcome` is NOT `rejected` and NOT `timeout_escalated`, which `"unknown"` satisfies. Deliberately not fixed with the row above: it changes what is sealed into an immutable chain, which is a subject of its own and not a line in someone else's PR |
