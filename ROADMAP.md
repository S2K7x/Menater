...
| **An approval answered in the console is interpreted as SILENCE** | `routes/approvals.ts` posts `{ approved, human_reasoning, approver: {…} }` to `engine.resumeWait`; `interpretApproval` reads `payload.decision`, `payload.approver` (a string) and `payload.reason`. Measured on the real pipeline with the payload the route actually sends: a human pressing **approve** yields `timeout_escalated`, the graph takes the `rejected` branch, the approved action is **not executed**, and the audit row — append-only and immutable by design — records `routing_outcome: "rejected"`, `action_taken: "none"`, `"reason": "Action refused by a human."` An explicit yes, written into the hash chain as a refusal, in the one place that cannot be corrected afterwards. It fails safe on the ACTION and it lies in the RECORD. Reachable only once an install leaves shadow mode, which is the only state where approvals exist at all. Not fixed with the reader above: it changes what the pipeline executes on the irreversible-action path, and which side of the boundary owns the vocabulary carries a second question — the route deliberately builds an approver record carrying `identity_source: 'console_self_declared'` (see the traps table) and `interpretApproval` discards it and writes `self_declared`, so a naive translation loses the documented value. **Visible since the reader fix**: the card now says `timeout_escalated` on a case a human approved, where before it said `rejected` on everything |
