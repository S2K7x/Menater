...
| **`normalize()` breaks its "never throws" contract on deeply nested JSON, unauthenticated** | Found by a DoS sweep on this same night and left for its own subject. `routes/ingest.ts` calls `normalize(mapping, await readBodyOrNull(req))` **before** `handleAlert` looks at the shared secret, and `normalize` does `JSON.stringify(v)` on a mapped field with no depth floor (`engine/transforms/normalize.ts:248`), under a header promising *"Never throws: a payload it cannot read produces missing identity fields, which `validateAlertSchema` turns into a named rejection."* Reproduced over a real socket with the console lock ON and **no `x-soc-token` at all**: a 117 kB body of `{"raw_log":[[[…60 000 deep…]]]}` — comfortably inside the 256 kB cap — gives `RangeError: Maximum call stack size exceeded`, logged as an uncaught error and answered **`500 {"error":"Maximum call stack size exceeded"}`**. Two defects in one line: a server-fault code for a sender fault, which is the trap `normalizeRuleInput` exists to prevent, and V8's internal message forwarded to an unauthenticated caller. `JSON.parse` in `readBody` accepts the nesting happily — the cap is on BYTES only. The fix is a depth floor in `normalize` and a named **400**, in the same shape as `BodyTooLarge` |
