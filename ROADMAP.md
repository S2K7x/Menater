...
| **The approve and reject buttons are disabled on every real case** | Found while closing the row above, and it is the other half of the same path: the console cannot answer an approval at all. `CaseView` computes `canSubmit = approver.trim().length > 0 && Boolean(execId) && !busy` with `execId = approval.execution_id`, and **`cases.ts` never writes that field** — it builds the approval from the `request` and `interpret` node outputs, neither of which carries it. Measured by driving the real pipeline to a live approval wait and reading the case the console would render: `state: 'awaiting_approval'`, `approval` present, `approval.execution_id: undefined`, so `Boolean(execId)` is `false` and BOTH buttons render `disabled`. Nothing says why: the other term of the same `&&` is the operator's own name, so a greyed-out button is indistinguishable from *you have not typed your identifier yet*. Only `demo.ts` sets the field (`'demo-done'`), which is why the screen looks right in demo mode. **And the identifier is the same disagreement as the row above, one field over**: `src/lib/api.ts` names the parameter `executionId`, `CaseView` passes a RUN id, and the route reads the path segment as a wait TOKEN (`store.waitByToken`) — the engine's tokens are `crypto.randomUUID()` and are not run ids, so even a populated `execution_id` would answer **404 « that approval is not open any more »**. Not fixed with the row above, deliberately: the choice between carrying the token to the browser (a capability, in a snapshot every open tab holds) and resolving a run id to its open wait SERVER-side (a new store method, hence a change to `pg-store.ts` that cannot be verified without a database — this environment has none) is a decision, not a typo. The second is almost certainly right: the Slack `resume_url` already identifies a run and never the token |
