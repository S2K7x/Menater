...
| **The snapshot is serialised and compressed once per REQUEST** | Measured on the real pipeline at the maximum window (100 alerts / 500 runs): the payload is **563.9 kB raw, 45.3 kB gzipped**, and `json()` spends **2.76 ms** in `JSON.stringify` plus **3.73 ms** in `gzipSync` — ~8.5 ms of blocked event loop per request — on an object the cache is handing back unchanged for up to 15 s. Ten tabs at the 5 s minimum refresh is ~17 ms/s of loop spent recompressing identical bytes, on the same loop the ingestion webhook runs on. The fix is to cache the serialised bytes beside the snapshot, not to make `gzipSync` async: the compression is not the waste, the REPETITION is. Not urgent — 17 ms/s is 1.7% of the loop — and it touches `respond.ts`, which every route shares |
