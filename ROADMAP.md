...
| **The database test button answers about a port the console would never dial** | `POST /api/settings/test/database` does `Number(body.port ?? d.port)` and hands it straight to `tcpProbe`, while `saveConfig` ten lines away already clamps the same value into `[1, 65535]`. The rule exists; the diagnostic — the thing an operator presses precisely when something is wrong — does not apply it. Measured through the real handler and the real client on Node 22.22.2: clearing the port field sends `0` (`Number('') === 0` in the `type="number"` onChange) and the button answers **200** *« Connection refused: nothing is listening on this port »* — a confident diagnosis about a port that save would have turned into 5432, so the operator goes and looks at their database. And `70000`, `-1` or `5432.5` make `createConnection` throw `RangeError ERR_SOCKET_BAD_PORT` **synchronously inside the promise executor**, which nothing catches, so the route falls to the last net in `app.ts`: **500** carrying Node's own *« Port should be >= 0 and < 65536 »*, logged as an uncaught error. A server-fault code for a sender fault — the trap `normalizeRuleInput` exists to prevent, one route over. The fix is a named **400** at the route rather than a clamp: silently probing 65535 when somebody typed 70000 answers a question nobody asked |
