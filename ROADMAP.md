...
| **`trace.chains[].payload` is shipped in full and read as a boolean** | The browser's only use of it is `if (!chain.payload)`, to decide whether the replay button is offered; `POST /api/replay` re-reads the payload SERVER-side from its own snapshot, so the five fields never need to travel. Measured at **7,883 bytes of 138,646 (5.7%)** on a 24-alert window — negligible once gzipped, which is why this is filed as hygiene rather than performance: the field that dominates it is `raw_log`, i.e. a second copy of attacker-composed text on the wire, for nothing. A `replayable: boolean` would say exactly what is read |
