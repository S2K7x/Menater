...
**The approval deadline has a clock.** `04-Action-Routing` suspends on `attente`
for `approval.timeoutMinutes`, and `server/engine/scheduler.ts` is the timer
that makes that number mean something: started in `api.ts` beside the poller,
one sweep at a time, `unref`ed, and it reads `getEngine()` on every tick so a
database configured from Settings is swept with no restart. The expiry takes
the `timeout` port — **never `main`** — so it escalates and executes nothing.
It had no caller for its whole life; see the traps table.
...
| **401 and 502 are poisoned status codes** | `lib/api.ts` turns ANY 401 into `AuthRequiredError` — "log in again" — and replaces the body of a 502/503/504 with a generic "the API is not responding" (those usually come from the Vite proxy and are not JSON). So a route that forwards an upstream's 401 answers a refused **model** key with *"Authentication required."* and sends the operator to the login screen; one that reports a named failure as a 502 has its sentence thrown away. Everything the operator can fix leaves the assistant as a **409**. **The 5xx half is fixed** — see the row below — so the 409 is now a belt-and-braces answer rather than the only one that works. **The 4xx half is fixed too** — see the row below. One half stands: a forwarded upstream 401 still sends an operator to the login screen, which is why the assistant's 409 remains |
| **A reason read under the one key somebody remembered** | The 4xx half of the row above, and the last one. `call()` read a failed request's explanation from `body.error` alone — and this server names a reason under **three** keys. Measured by driving the real handler through the real client: `POST /api/rules` and `PUT /api/rules/:id` answer `400 {"problems":[{field, detail} × 5]}`, one sentence per thing wrong with a tuning rule, and all five became *"The console server answered 400 with no explanation."* `POST /api/settings/test/database` lost *"Host missing."* the same way. The worst is `POST /api/approvals/:token/resume`, which answers `{ok:false, detail}`: somebody has just answered the question the pipeline stopped to ask, and what was discarded named the cause AND the consequence — *"Without it the run will time out and the alert will be escalated."* **The screen had the code to show it and had never once received it**: `RulesPage.save()` reads `err.problems` and renders one line per entry, under a comment saying that showing "invalid" instead would throw the useful half away — but `call()` threw a plain `ApiError`, so that branch had been dead for its whole life. Dead code that LOOKS load-bearing, on the screen where a team writes the rules deciding what stops reaching a human. Two more shapes fell out of writing the guard, and both were live: the 4xx branch had no non-empty check, so an `error` of `"  "` put a **blank banner** on screen — the 5xx branch above had that guard and nothing else did, which is why there is now ONE reader for both — and `body?.error` was never tested for being a string, so `/api/mcp`'s JSON-RPC envelope (whose `error` is an object) was one browser call away from printing `[object Object]`. The rule: **`error` wins when several keys are present** (the settings route composes a human sentence out of the same list, and a raw join must not displace it), a blank or non-string reason is not a reason, and the structured list travels on the error so the screen that renders it receives it |
| **A generic sentence written for the only thing that could answer it** | `lib/api.ts` replaced the body of EVERY 502/503/504 with *« The console server is not answering. Start it with `npm run serve`. »* — true of the Vite dev proxy, which was the only thing that could answer those codes when the rule was written. Measured on this repository's Vite (8.2.1) against a dead upstream: **502, `content-type: text/plain`, an EMPTY body**, so there is genuinely nothing else to say. But the console answers 503 too, and never for that reason: **no database configured** (the rule store cannot be read, the engine is not mounted, so the Rules tab, the Health tab's injection button and an approval being answered all 503), and **a configured database that refuses** — where the sentence thrown away is the one `describePgError` exists to recover, since `pg` reports `ECONNREFUSED` with an EMPTY message. So the screens whose whole job is to say what is wrong sent the operator to restart a server that had just answered them, and the fix — *Settings → Database* — was named in a string that never left the process. **The rule was already written forty lines down in the same file**: `pwnedRange` cannot use `call()` (its route answers plain text) so it reads its own errors, and says *« a JSON body here carries the named reason, and throwing it away for a generic sentence would lose the only useful half of the answer »*. Same family as `readCapped` being careful about everything the inbound cap was not: the rule existed, on one route out of twenty. `call()` reads the body BEFORE deciding, and a blank `error` is not a reason — an empty banner says less than the generic sentence does. The one route whose envelope is its own answers **200** instead: `POST /api/simulate` carries `{ ok, response }` and the Health tab prints `response` where the answer goes, exactly as the catch two lines below it already did for an engine that throws. A status code is not the place to say *there is no pipeline to inject into* |
...
| **A content-keyed identity on an append-only dedup table** | `soc_ingested_alerts` is keyed on `alert_id` with **no expiry**, so an id decides *for ever* whether something may enter the queue again. J0.1's first draft keyed a promoted finding on the flaw alone — vulnerability, method, route, file — which is right for a report's own status tracking (`finding-status.ts` does exactly that, and deliberately drops the line number because it moves). It is wrong for an EVENT: a flaw fixed in January and regressed in June would be answered `duplicate, skipped` about a case closed six months earlier, silently, under a green check — the permanent hole this table is otherwise full of. **A queue is a journal of what happened, not a picture of what is true**, so the SCAN RUN is part of the key: two presses on one finding of one scan stay one case (the protection that is actually wanted), and next month's scan can raise it again. Whether the same unfixed flaw is worth promoting twice is the operator's call, made by pressing the button |
| **A shared primitive makes a `getByRole` somewhere else ambiguous** | `Explain` renders its bubble with `role="note"`. Adding one control that uses it to `FindingCard` turned two pre-existing `getByRole('note')` queries — written when the card had exactly one note — into *found multiple elements*. The tests were RIGHT and the query had merely stopped being unique, so the fix narrows it (`p.vp-notice[role="note"]`, role still asserted) rather than relaxing it. Worth knowing before it reads as a regression: **adding a shared component to a screen changes what every role query on that screen matches**, and the failure surfaces in a file you did not touch |
| **A sweep that checked one of the two catalogues** | `n8n-removed.test.ts` exists to stop a name outliving the thing it named, and it walks `src/i18n/console.ts`. **`server/i18n.ts` was never in it** — the diagnostic, the chain notes and every route answer — and it had kept the vocabulary without keeping the name, so a test forbidding `n8n` passed over all of it: `api.webhookUnreachable` told an operator to *"check that 01-Ingestion is **published** and the **instance** answers"*, about a publish step this product does not have and a server it does not dial. Ten server strings and four console ones were referenced by **nothing at all** — the state a typed catalogue cannot see, because it refuses a key added on ONE side and both sides of a dead key agree perfectly (same shape as R42). The rule that finds them searches the bare key NAME across product code, and it taught two lessons while being written. **A test whose own prose vouches for the strings it hunts**: the first draft listed the seven dead keys in its header to explain them, and went green on a comment describing the defect — tests and comments are stripped now. **And a catalogue left in its own search corpus makes every key vouch for itself**: the second draft's console-catalogue rule passed over four dead entries for that reason alone, which is the defect rebuilt one notch in, the family this table already carries as the scan report that took green when coverage was UNKNOWN. Both catalogues are excluded from the corpus; the rule under-reports on names that collide with ordinary words (`answered`, `keyMissing`) and never flags a string that is really referenced |
| **A capability list written in prose, beside the catalogue it describes** | The assistant's system prompt opened with *"you have read-only tools over this installation's live data: the alert queue, one alert in full, the execution chain behind it, the tuning rules, the metrics, the health report and the setup state"*. Seven, and true when written. `TOOLS` grew to **fourteen** and that paragraph did not, so `search_alerts`, `get_attention`, `explain_term`, `find_similar`, `explain_verdict`, `get_timeline` and `test_rule` were sent as schemas and named nowhere in the block whose entire job is to say what the model may reach for. The schemas alone are not the same thing: they answer *how do I call this*, the prompt answers *what am I*. The sharpest loss is `explain_term` — it holds THIS console's glossary and exists so that "what is shadow mode?" is not answered from training, three lines under an instruction not to answer from memory. **Prose cannot be checked; a list of names can**, so the prompt now names all fourteen, grouped by question, and the test fails in BOTH directions — a tool the prompt does not name, and a name whose tool has gone (which costs the model a step to discover). Measured cost: the cached half goes 3,393 → 4,009 characters, in the block the breakpoint sits after, so it is a cache write and not a per-request bill. Three operator-facing sentences had frozen at the same seven — the panel's note, the Settings blurb and the Guide, whose *"Seven lookups"* was flatly false. They describe REACH now and a test forbids the COUNT: an operator told the assistant does seven things never asks it the other seven questions. Same family as the n8n sweep and as "a number that skips" |
...
| **A reply that chooses where the next request goes** | Every secret this console sends outbound travels in a request HEADER — the poller's managed credential, in a header an operator named, at an address an operator typed; the enrichment node's provider keys; Anthropic's `x-api-key` in the assistant. `fetch` follows a redirect by default, and the spec strips exactly ONE header across an origin. Measured on Node 22.22.2, two servers on two ports, one `302`: `authorization` **absent** at the second host, `x-api-key` and `x-soc-token` **delivered**. So `Authorization` was safe by an accident of the specification and everything else went wherever the host we dialled pointed — the second half of the exfiltration primitive `isManagedCredential` exists to remove, reassembled by the remote end: the closed catalogue decides WHICH secret may leave, and nothing decided WHERE. The rule was already written down — `intel/lookup.ts` carried `redirect: 'manual'` and a comment saying a 3xx "would mean the endpoint moved, which is a thing to notice, not to obey" — on **one call site out of eight**, including the `pwnedRange` relay forty lines below it in its own file. That is the sentence `http.ts` already makes about the deadline, come true a second time: *a primitive that has to be re-typed by hand is a primitive that will eventually be forgotten.* It lives in `fetchWithDeadline` now, and it is not "refuse every redirect": a hop that stays on the host the caller NAMED is followed, because a trailing-slash 301 is the commonest redirect there is and the credential travels no further than it already had. Refused and SAID: another host, a same-host drop out of TLS, and any redirect on a write (`notify` POSTs the alert's own text, so a 307 would replay that body elsewhere and a 302 would silently turn it into a GET — there is no method rewriting here because there is no redirected write). **And the tests taught the sharper lesson**: eleven of them drove an injected transport, which ignores `redirect: 'manual'` and hands back whatever Response the fixture wrote — so deleting that line left all eleven GREEN over a reopened hole, and against the real `fetch` undici would have followed the redirect before this code ever saw a 3xx. A fake cannot vouch for an option the fake does not implement; two real sockets can, and do |
| **A refusal reported as three other things, none of them the refusal** | `readBody` has capped the request body at 256 kB since the request-body trap two hundred lines above was written down, and the cap itself worked. What nobody checked is the SENTENCE, and it took three shapes, each wrong in its own way. **`PUT /api/ingestion/policy` answered 200 with the policy.** It read its body as `readBody(req).catch(() => null)`, and `normalizePolicyInput` treats an absent body as *change nothing* and reports success — so a save refused for its size came back as a save that landed, on a settings route, in the product whose Tracking tab exists to expose a failure that shows green. **`POST /api/findings/promote` and the two ingestion endpoints** — same `.catch` — answered a verdict about the body's SHAPE, *`scan_run_id`: a non-empty string is required*, naming fields nobody had read: a diagnosis nobody made, sent to the one caller that cannot ask a follow-up question, an appliance. **And the fifteen routes that did not catch** fell to the last net: **500** carrying `Corps de requete trop volumineux.` — a server-fault code for a sender fault, in French, in an English-only product, logged as an uncaught error. The general rule is the one the `catch` hides: **a `catch` that maps every failure to one value maps the failures you knew about and the ones you did not**, and the second kind is what arrives dressed as good news. `readBody` now throws `BodyTooLarge` carrying the cap it enforced, `app.ts` words it as a **413** through `humanBytes`, and the three tolerant callers take `readBodyOrNull`, which keeps `null` for a body that could not be READ — a socket that died mid-transfer tells a route nothing — and lets a refusal WE made travel on. Note where the answer already existed: `readCapped` in `poller.ts`, the same cap pointing OUTWARD at a polled source, was careful about every one of these from the day it was written. The mirror of a rule is not the rule, and the inbound half had never been read next to it. `humanBytes` lives in `respond.ts` now and both directions import it, because two spellings of one number is how the two start disagreeing |
| **The lock that locked the operator out** | `requiresAuth` answered `true` for `/`, and the login form is a React component INSIDE the bundle it was refusing (`LoginScreen` in `src/App.tsx`). So setting a console password made the console unopenable: a fresh browser got `401 {"error":"Authentication required."}` in place of a page, and — sessions living in memory — a container restart did the same to everyone already signed in. The only way back was to edit `config.json` on disk. **It could not show in development**, where Vite serves the interface and only the `/api` calls ever reach that function, so this is the `/api/*`-served-the-SPA defect the other way round, with the same note: only broken in Docker, for want of a `dist/` to serve. The rule the fix states is that **the lock guards data, not the page that asks for it** — everything this console knows is answered under `/api/`, and a path outside it can only be served by `serveStatic`, which refuses `/api/` itself and cannot leave its root, so the shell carries nothing an unauthenticated visitor could not already read in this repository. Written as the whole subtraction (`/^\/api(\/|$)/`) and not as a list of file names, because a list is the exact-match set that already cost this project every ingestion endpoint — one Vite output away from going stale. The plausible wrong fix is worth naming: exempting by EXTENSION (`.js`, `.css`, `.html`) leaves `/settings` — a screen, not a file — locked, and opens `/api/settings.json`; a mutation test carries both |
| **A human's approval, filed as a refusal** | `interpretApproval` returns `{ outcome, proposed_action, approval }`, and `cases.ts` read a top-level `approved`, `approver` and `human_reasoning` that all live one level down under `approval`. `approved === true` was therefore false for **every answer a human ever gave**, so the incident card showed an orange warning — *« Declined by unknown »*, *« No reason given »* — on a case whose action had just been EXECUTED and whose audit row said approved. Two more on the request beside it: the reasons someone was asked are `approval_triggers` (read as `triggers`, so the **« why you are being asked »** block had never once rendered — the approver authorised a containment action without seeing the three sentences justifying it), and the deadline was on the request under **no name at all** — `ttl_minutes` sitting next to it is how long an isolation LASTS — so the card printed a hardcoded **30**, which is the DEFAULT of `approval.timeoutMinutes`: right on a fresh install, silently wrong for everyone who changed it. **A default that agrees with the default is the hardest kind to see.** None of this is cosmetic: `human_disagreement_rate` counts `outcome === 'rejected'`, and § Measurement calls it *the one that gates leaving shadow mode* — so it read **100** the moment anybody approved anything, on the single number that decides whether this console may ever act. **Why `pipeline-to-case.test.ts` — the file written to END this class of defect — was green:** the whole `request`/`attente`/`interpret` branch sits behind the `shadow` if-node, and every test in that file runs in shadow mode, which is the default. A test that closes a trap has to be driven through **the state where the trap lives**, and this one lives only on installs that have gone live. `cases.test.ts` stayed green throughout for the older reason: its fixtures are hand-written in the READER's vocabulary. The fix's ORDER is part of it — `interpret` is the node that DECIDES and `rejected` / `timeout` are the branches taken because of its answer, so reading them last let a branch overwrite the decision, and a card that cannot disagree with the branch cannot show a defect upstream of `interpret`. There is one, and it is in ROADMAP § 7 |
| **A duplicate gzip cannot reach, and a display string handed back as input** | One field, `trace.chains[].payload`, and it was wrong twice. It carried the five fields of the alert as the webhook received it to every open tab, where the only thing ever read off it was `if (!chain.payload)` — `POST /api/replay` re-reads them on the SERVER side out of the same snapshot, because the browser posts an `alert_id` and nothing else. **The first lesson is that « negligible once gzipped » is a claim, not a property.** It was written in `ROADMAP` § 7 and never measured: `raw_log` dominates the five fields and the case already carries it, so this was a second copy — and the two copies sit **85,644 bytes apart** at the default window, 351,637 at the 500-run ceiling, far outside DEFLATE's **32,768-byte** sliding window, so gzip cannot collapse one onto the other. Measured through the real pipeline: 162,113 → 144,696 raw (−10.7%) and 15,952 → 13,608 gzipped (**−14.7%**) at the default window — the compressed saving proportionally LARGER than the raw one, which is the opposite of what the assumption predicted. A duplicate is only free while it is inside the window; past that it is paid for in full, twice. **The second is the `shortLabel` trap again, and it had broken the feature.** Those five fields are not shown anywhere — they are re-INJECTED at the entry point — and they were stored through the incident card's display fallback, so an absent observable became an em dash. The entry point's validator refuses a destination that is present and not an address, and is right to: `400 — invalid_dest_ip: not an IPv4/IPv6 literal`. **So the Tracking tab's replay button — the documented repair for a broken chain — refused every alert carrying no destination address, which the contract says is most of them.** It failed at the one moment it is pressed, and nothing upstream looked wrong: the value was correct for a screen it never reached. Found by driving `POST /api/replay` through the real handler and asserting on what the ENGINE received, read back out of the run journal — the route's own sentence says « replayed » either way, and the pipeline's 400 was inside the `detail` nobody had ever read. **A string shortened, padded or dashed for display is not an input**, and the moment such a value travels the two need separate fields — the rule J0.1 wrote for `ScanState.launchedTarget`, unapplied one file over |
| **A cache that stops one wall of work and pays the next one over** | `snapshot()` hands the SAME object to every caller for up to fifteen seconds — that is what its cache is for — and `json()` then re-ran `JSON.stringify` and `gzipSync` over it on every single answer. Measured on a window of the documented size (563,315 B raw, 48,396 B gzipped): **6.12 ms of blocked event loop per request**, on the single thread that also serves the ingestion webhook. A cache that saves the DATABASE walk and pays the ENCODING again is half a cache, and the half it skips is the half nobody times. The fix is a memo keyed on the body's **identity** (a `WeakMap`, so the bytes are collected with the body): a body that changed is necessarily a different object, so there is no invalidation anybody can forget — the same argument as the inventory index, and the reason it is not keyed on content, which would mean hashing bytes we have not produced yet. Repeat answer: **0.1-0.6 us**. Two things the shape of it teaches. **The saving was worth nothing until the caller stopped copying**: the route composed `{ ...snap, refresh_seconds }`, a new object per request, so an identity memo would have missed every time — a per-request nonce in a cached prefix, rebuilt in the response layer. The cadence travels on the snapshot now. And **the compressed form is kept BESIDE the raw one, never instead of it**: the next caller may send `accept-encoding: identity`, and a memo that let one form displace the other would answer it with bytes it cannot read. The new invariant this creates, and it is the price: **a route must never answer twice with an object it mutated in between** — no route does, and a test pins the contract |
| **A rule that was written down in one half of the product** | Six screens of this console answer a question by writing a sentence into a panel — the connectivity diagnostic, the test-alert injection, the MCP self-test, the tuning-rule dry run, the database probe, the chain replay, a manual lookup, a save — and the assistant's whole output arrives that way. The operator presses, waits, and a line appears somewhere on the page, often far from the button and sometimes forty-five seconds later. **Nothing announced any of it**: `role="status"`, `role="alert"` and `aria-live` appeared **zero** times in `src/components/`, while `src/vulnpipe/` — the other half of the same application, built by the same hands — used them twenty-two times and had written the rule in a comment beside one of them: *« `role="status"` and not an alert: it is the result of something the person just did »*. For somebody working with a screen reader a refusal and a success are then the same thing, silence — the failure this console exists to expose, committed against itself. Three things the fix had to get right, and each is the general lesson. **The region has to pre-exist**: one created in the same breath as its first message is announced by some screen readers and missed by others, so `Announce` is a WRAPPER that is always in the DOM and not an attribute on a conditional banner — « sometimes » is not a guarantee. **Polite, never assertive**: somebody who just pressed a button is waiting for this sentence, and interrupting them to deliver what they asked for costs the reading they were in the middle of. **One region per answer slot**, because `role="status"` is atomic: the injection's verdict and the follow-up line that advances every few seconds for forty-five seconds would otherwise re-read the verdict at every step. And the boundary is half the fix — a STANDING condition must not be in a region at all (a broken chain read off the snapshot, a warning about a setting's value, the guarantee above the Lookup field), because the console re-renders on every poll and a permanent announcement is the permanent alarm the `diagnostic_probe` row above already refuses. Two of the nine tests claim that side and pass before AND after, on purpose. One place it is deliberately NOT applied: `.soc-actions` is a flex row with a `gap`, so an always-present region there is an extra flex item — announcing the pipeline variables' save confirmation means moving it out of the row, which is a visual change and not an accessibility one |
| **A component that keeps its contract, and a caller that owes the other half** | `Icon` says so in its own header: a captionless glyph is DECORATIVE, it carries `aria-hidden`, and the label is « porte par le texte voisin ». It keeps that promise exactly, and it is right to — it is what stops « New rule » being announced as « check New rule » on every button in the console. But a promise kept by the component is an obligation moved to the CALLER, and nothing checked the callers. Across **155** `<Icon>` uses, exactly two sit in a control with no neighbouring text and no label: **edit** and **delete**, on every tuning-rule row. Measured in real Chromium on the markup the component actually renders — `page.accessibility.snapshot()`, not deduced from the source — the row announced `"Disable", "", ""`. So a screen-reader user walking the rules list heard *« Disable, button. button. button. »* per row: three controls, two anonymous, and the way to learn which one DELETES was to press it. WCAG 2.2 § 4.1.2, level A, on the screen where a team writes the rules deciding what stops reaching a human. **The rule already existed one file over** — `Assistant.tsx` gives its icon-only send button an `aria-label` — which is the live-region row above, the `readCapped` row and the `redirect: 'manual'` row a fourth time: *the mirror of a rule is not the rule*, and a contract a component documents about ITSELF is not a test of the people using it. Two things the fix had to get right. **The name carries the rule**, because the pair repeats once per row: twenty identical « Delete » buttons name nothing, and a test refuses the constant label rather than trusting the next edit. **And the boundary is half of it**: the plausible wrong answer is a `title` on every `Icon`, which announces the drawing BESIDE the word it decorates and buys noise with the same keystroke — a test claims that side and passes before AND after. What found it was a DOM sweep over the mounted screens asking for the accessible NAME of every control, using Testing Library's own computation rather than a hand-read of `aria-label`, which would have agreed with whatever the fix happened to write |
| **A test that asserts the ENVIRONMENT refuses something** | `persistent-cache.test.ts` checks that a scan does not fail when the disk refuses the cache write — the right behaviour to check — and provoked the refusal by naming `/nonexistent-root/cache.json` and **assuming nobody could create it**. That is a claim about the UID running the suite, not about the code. Measured by driving the real `saveCache` under two UIDs on Node 22.22.2: at uid 65534 it is `written: 0, why: EACCES` and the test passes; **at uid 0 it is `written: 1, why: null`** — `mkdirSync(..., { recursive: true })` simply creates the directory — so the test fails, and what it is reporting is a SUCCESS of the code under test. Two costs, and the second is the one nobody was looking for. The suite passed or failed on **who ran it**: green on CI (`ubuntu-latest` runs as `runner`) and in both Docker images (`USER node`), red for anyone running it as root — a container shell, a devcontainer, this repository's own nightly routine. An intermittent test is a test people learn to ignore, which is the lesson this project already paid for twice (the developer's `config.json`, the developer's `.vulnpipe/` cache) and once more for a test that really slept. And **it wrote `/nonexistent-root/cache.json` at the filesystem root** — outside its own tree, under a file header promising that *« chaque test écrit dans son propre dossier temporaire »*. The header vouched for every test in the file, and the one that escaped it is the one that escaped the file. The fix is that a refusal has to be **structural, not a privilege**: a path under a REGULAR FILE (`.../occupied-by-a-file/sub/cache.json`) is `ENOTDIR` for every UID, and it stays inside the test's `mkdtemp`. Note the plausible wrong fix, which was probed and rejected: `chmod 0o500` on a directory **succeeds** for root, so it rebuilds the same defect inside its own repair. The general rule: **a test must provoke the failure it is testing, never borrow one from the machine** — and if the premise can stop holding, assert which refusal you actually got (`why` contains `ENOTDIR`), or the test goes green again one day without exercising anything |
| **A lockfile that described the OTHER half of the product** | `VulnPipe/package-lock.json` carried the console's devDependency list in its root `packages[""]` block — `@testing-library/react`, `react`, `react-dom`, `vite`, `jsdom`, `concurrently`, `@vitejs/plugin-react`, `@types/react`, `@types/react-dom`, `@testing-library/user-event`, ten packages this half declares nowhere, against the three it does. Somebody had run an npm command in the wrong directory. **Nothing said so, and that is the whole defect**: `npm ci` reconciles against `package.json`, so it installed the right tree, and the suite, the typecheck and CI were green over it for as long as it existed — the lockfile is the claim under test *precisely because* CI runs `npm ci` and never `npm install`, and a root block describing another package makes that claim about the wrong package. What it cost was measured, not reasoned: it froze the transitive tree around a resolution nobody could refresh, so `npm update fast-uri hono qs --package-lock-only` pruned **595 lines before changing a single version** — the one command that closes a published advisory looked like a rewrite nobody would dare review, and **three production advisories sat behind it** (`fast-uri` 3.1.5, four separate CVSS 7.5 SSRF and host-confusion advisories, reachable: probed with a CJS-cache instrument, it loads **3 modules on both SDK entry points VulnPipe imports**, `server/mcp.js` and `client/index.js`, via `ajv`; `hono` and `qs` load **zero** on every one of them, because VulnPipe speaks stdio and never imports the HTTP transport — the control proves the instrument: `server/streamableHttp.js`, which VulnPipe does not import, loads `hono`). Two things the fix had to get right. **The obvious remedy was the wrong one and would have failed silently**: the previous night's journal recommended bumping `@modelcontextprotocol/sdk`, and `1.30.0` **is the latest version** — every one of the five advisories is inside a caret range the tree already declares, so the fix is a lockfile refresh and not a manifest change. And **an instrument that reads zero everywhere is a broken ruler, not a finding**: the first probe used an ESM `load` hook, which does not see CJS `require()` chains, and reported `fast-uri: 0` — the exact opposite of the truth, on the one advisory rated high. Same family as a contrast measured during the theme cross-fade. `src/lockfile.test.ts` pins the rule on **both** halves from one place, because the mirror of a rule is not the rule — this table has paid for that four times — and the console's three assertions pass before and after, on purpose |
| **Three parties, two already agreed, and the one in the middle invented a vocabulary** | The browser posts `{ decision, approver, reason }` to `/api/approvals/:runId/resume` (`src/lib/api.ts`) and `interpretApproval` reads `payload.decision`, `payload.approver`, `payload.reason`. **Both ends of the conversation were right**; `routes/approvals.ts`, sitting between them, translated into a third form — `{ approved, human_reasoning, approver: {…} }` — that nothing on the far side read. Nothing failed: `p.decision` was `undefined`, so the transform's own rule (*anything that is not exactly `approve` or `reject` is silence*) turned a human's explicit yes into `timeout_escalated`. Measured through the real route over the real pipeline: the approved containment was **never dialled**, and the audit row — append-only, immutable, the product's only tamper-evidence — recorded `routing_outcome: "rejected"`, `"reason": "Action refused by a human."` about a yes. It fails safe on the ACTION and lies in the RECORD, in the one place that cannot be corrected afterwards. Three general lessons. **A mismatch between two correct endpoints is invisible from either one**, and from every unit test of either one — only a test that drives the whole boundary sees it, which is why the file that closed this drives the REAL route and reads the ENGINE's conclusion out of the run journal rather than the sentence the route printed. **A decision is a WORD, never a boolean**: `approve`, `reject` and *nobody answered* are three facts, and the moment the route reduced them to `approved: boolean` the third one had nowhere to live — so an unrecognised answer is refused at the route with a named 400, because forwarding it spends the token (`resolveWait` is irreversible) and files a silence that never happened. **And the transport sends a CLAIM, never a RECORD**: the route was building the approver record itself, `identity_source` included, on the side of the boundary a payload can reach — a transport able to author `signature_verified: true` writes an authentication that never happened into the hash chain. The guardrail mints it now, and the value is `console_self_declared`, which this table and `dashboard/README.md` had documented for so long that nobody noticed **nothing produced it**: the route built it and `interpretApproval` threw it away |
| **An identifier that is also a capability, and a control disabled for two reasons at once** | The other half of the row above, and it had made the whole path unusable: with the payload finally right, **the approve and reject buttons were still disabled on every real case**. `ApprovalPanel` computes `canSubmit = approver.trim() && Boolean(execId) && !busy` with `execId = approval.execution_id`, and `cases.ts` never wrote that field — it builds the approval from the `request` and `interpret` node outputs and neither carries it. Measured by driving the real pipeline to a live wait and reading the case the console would render: `approval` present, `execution_id: undefined`. **A control can be disabled for two reasons, and the operator is told neither**: the other term of that `&&` is their own name, so a greyed-out button is indistinguishable from *you have not typed your identifier yet* — and only `demo.ts` set the field, which is why the screen looked right in demonstration mode, i.e. in the only place anybody reviews it. The fix is not the obvious one. The route read the path segment as the engine's wait TOKEN (`store.waitByToken`) while the client has always named the parameter `executionId` and passed a RUN id, so the plausible repair is to ship the token onto the case and be done. **A wait token resolves an approval exactly once and irreversibly: it is a VERB, not a name.** The case it would ride on is answered by the assistant's `get_alert` (`sections: ['approval']`) to a model whose context also holds attacker-composed log text, and by `POST /api/mcp` to any client holding a read-only bearer token — the catalogue kept closed and verbless *precisely* so that an injected instruction arrives somewhere with no verb in reach. That fix would have posted one through the fence. Run ids already travel there on every stage line, so naming the run adds nothing; `RunStore.openWaitOfRun` resolves it on the server, `Engine.resumeRun` is its only caller, and the token never leaves the process. Two things fell out of it. A wait already settled is **not open**, so the second of two simultaneous presses is now told *« that approval is not open any more; the first answer stands »* instead of **200, decision relayed** about a decision that was not the one recorded. And the test that closes it had to read its identifier **off `buildCases`** rather than naming a run id: a test that names the id itself passes over a card that carries none, which is the whole defect — 10 of that file's 13 tests are red before the change for exactly that reason |
| **A guard that names the door instead of the work behind it** | Two of them, found in one night's sweep, and each is a rule that was right about the path somebody was looking at. **The console did its most expensive read for callers it had just refused.** `/api/ingest/:source` and `/api/webhook/soc/alert` sit outside the access lock on purpose — an appliance has no browser session — and `handleAlert` turns four kinds of caller away before the engine is reached: mode off, no secret configured, wrong token, no engine. `routes/ingest.ts` then called `invalidate()` **unconditionally**, under a comment that is simply true of the case it was written for, *« An alert received changes what the console must show »*. `invalidate()` drops `cache` AND `inFlight`, i.e. both properties `snapshot.ts` exists to provide, over the rebuild its own header calls *« the most expensive thing the console does »*. Measured over a real socket with the lock ON and no credential of any kind: **30 unauthenticated POSTs answered `401` bought 30 of 30 rebuilds**, once per request, with no rate limit, on the single thread that also serves the alerts. The attacker sees a 401; the operator sees a console that got slow — this product's defining defect wearing the other mask. **And the status code cannot be the test, in both directions**: three of those refusals are `503` while the PIPELINE answers a 503 of its own when dedup is unavailable, and a `400` for a missing field is an alert that DID run and shows in the Tracking tab. So the answer is a field set where the engine is actually started (`WebhookResult.ran`), never a code read back, and never a reason fished out of `body` — that is *« a node's output under a field name you remembered »*. **The MCP cap counted `tools/call` and not `resources/read`.** Same shape, two files over: `rateLimited`'s comment says it exists because *« an agent in a loop is exactly the client that will call them a thousand times without noticing »*, and `readResource`'s says *« every branch goes through `runTool`, NOT around it »* — two accurate comments describing one uncounted path to the same fourteen getters. Measured: 400 `tools/call` → 120 served; 400 `resources/read` → **400 served, 0 refused**. And in bulk, because a JSON-RPC batch is ONE request dispatched with `Promise.all`: a **179 kB batch of 2,000 `resources/read` served all 2,000 and blocked the loop for 123 ms**, the `Promise.all`-over-the-whole-window trap rebuilt inside a protocol handler. The general rule is the one both halves state: **a limit, a lock or a cap must be attached to the WORK, not to the name of the route that happened to reach it first** — and the way to find the second door is to grep for the other callers of the thing the guard protects, which is the *« fix applied at the front door and not at the two buttons behind it »* row a third time. Two boundaries were claimed on purpose rather than left implicit, by tests that pass BEFORE and after: an alert that got past the secret must still invalidate (or « the queue does not show what just arrived » replaces « the console is slow », which is worse), and `completion/complete` must NOT be metered, because it is one call per keystroke and an alert id is eighteen characters |
| **A cap on BYTES, in front of a limit that is about DEPTH** | `readBody` has refused a request body over 256 kB since the request-body trap above was written, and that cap is on bytes — which says nothing about the shape inside them. **`JSON.parse` is iterative in V8 and `JSON.stringify` is recursive**, so the parser hands the rest of the process structures the serialiser cannot survive: measured on Node 22.22.2, parse is still fine at **200 000** levels while stringify gives up at **4 165**. A 117 kB body of `{"raw_log":[[[…60 000 deep…]]]}` therefore walked through a cap that was working perfectly, and `normalize()` — under a header promising *« never throws »* — died on `JSON.stringify(v)`. Reproduced over a real socket with the console lock ON and **no `x-soc-token` at all**: **`500 {"error":"Maximum call stack size exceeded"}`**, a server-fault code for a sender fault *and* V8's internal message forwarded to a caller holding no credential, at the one endpoint that faces the Internet through the tunnel. **The second caller was the expensive one, and it is the « grep for the other callers » rule a fourth time.** `pollSource()` also calls `normalize`, and there the `RangeError` escaped the function, so `fail()` — which records the error, backs the source off and puts it on the Ingestion tab — never ran. It escaped `tick()` too, which the timer invokes as `void this.tick()`, and **an unhandled rejection from a timer terminates a Node 22 process** (measured, not assumed). One over-deep item from a polled source kills the console, on every interval, for ever. Three things the fix had to get right, and each is the general lesson. **The floor is on the WHOLE payload, never on the fields the mapping reads**: `extensions` keeps every unmapped vendor key whole, so a guard on the mapped fields alone refuses `raw_log` and waves the same nesting through under any other name — and the run journal (`pg-store`), the model prompt (`decision.ts`) and the assistant's fence (`sanitize.ts`) all serialise what arrives. **The cap is a POLICY NUMBER, not a measured maximum.** The obvious move is to set it just under what V8 allows; that 4 165 is a property of the stack left at the moment of the call, and the same probe under `--stack-size=2000` answers **8 500**, so such a floor moves with the machine it runs on. 64 is chosen against data instead — the nine injection scenarios are **2** levels deep, a Wazuh alert carrying MITRE arrays and agent metadata is **5**, and the deepest JSON this repository ships is **5**. **And the check refuses on the way DOWN.** The obvious way to write it is to measure the depth and compare after, which walks the whole structure and overflows on exactly the payload it exists to refuse — the defect rebuilt inside its own repair, the family this table already carries as the scan report that took green when coverage was UNKNOWN. Checked while descending, refusing a 200 000-deep body costs sixty-five steps, which is also what bounds the work one request can buy. **The test that claimed the last of those was too weak, and mutation is what said so**: it asserted *« the guard does not recurse »*, and a RECURSIVE check that compares before descending is bounded at `limit` frames and passed it happily. What fails it is measure-then-compare, in either form — so the test and the comment now say that, and not the thing that merely sounded like the rule. One consequence worth the line: `unusable` on a poll outcome had exactly one cause when its sentence was worded, *« carried no alert id and could not be read »*, and now has two — a sentence must not be reachable from a state it does not describe, so it names both rather than asserting the one it used to be sure of |
| **A `catch` that turned an unreadable body into an empty one** | The inbound mirror of the **« A body that could not be READ, reported as an empty body »** row above, and the last unguarded reader in this API. `readBody` mapped a `JSON.parse` failure to `{}` under a header vouching that this held *« because EVERY route validates what it received »* — a claim nobody had measured, and false in **both** directions. The routes that do not validate read `{}` as *change nothing* and reported success: `PUT /api/ingestion/policy`, `PUT /api/settings`, `PUT /api/credentials` and `PUT /api/workflows/variables` all answered **200 with the settled value** for a truncated body — **a save that never happened, reported as a save, on the four screens where an operator changes things.** That is precisely the defect the previous fix at this door removed for the SIZE refusal, still live one branch over for the shape. The routes that DO validate were no better: an appliance posting `application/x-www-form-urlencoded` by mistake — the commonest ingestion misconfiguration there is — was told its alert was missing `alert_id`, a diagnosis nobody made, to the one caller that cannot ask a follow-up question; and `POST /api/auth/login` answered **« Wrong password. »** and spent one of the eight attempts, so a client with a broken serialiser **locks the operator out of their own console**, blaming a password nothing had read. Three more things the shape of it teaches. **A guard written for a state it can never see is dead code that looks load-bearing**: `mcp.ts` opens with `if (body === null || typeof body !== 'object') rpcError(PARSE_ERROR, 'Body is not JSON.')` — written for exactly this case, unreachable for it, because `{}` IS an object. **A body that parses is not therefore readable**: `null`, a number and a string are valid JSON naming no field, and `POST /api/diagnostics` and `POST /api/simulate` read a field off them and answered **500 carrying V8's own message**, a server-fault code for a sender fault. And **the rule was already written, in the other half of the product** — `VulnPipe/src/orchestration/webhook.ts`'s own `readBody` refuses both states by name, under a comment saying that silently substituting `{}` *« would be filling a gap with a default, which this product refuses »*. Fifth recurrence of *the mirror of a rule is not the rule*. `MalformedBody` carries WHICH of the two it was, because one sentence for both would be reachable from a state it does not describe; the empty body stays `{}` (a dozen routes take none) and a top-level ARRAY stays itself (the MCP batch is the one this API accepts, where VulnPipe refuses arrays — same rule, different closed set) |
| **A promise with no clock behind it** | Everything around the approval timeout was built and correct, and it had never once run. `Engine.sweepExpiredWaits()` turns an unanswered approval into an escalation; the `timeout` port is wired to `escalate-timeout` in `04-Action-Routing`; `cases.ts` reads that node and sets `outcome: 'timeout_escalated'`; `sql/10-engine.sql` even carries the partial index the sweep query needs (`ON soc_run_wait (deadline) WHERE resumed_at IS NULL`), under a comment saying *« le balayage des echeances ne regarde que les attentes ouvertes »*. **A schema written for a periodic sweep, and no sweep.** The only callers were `engine.test.ts` and `integration.test.ts`; `setInterval` existed in `ingest/poller.ts` and in `auth.ts` and nowhere else in the process. So the message posted to a human said *« no answer within 30 minutes and the alert is escalated »*, silence did nothing, and the run stayed `waiting` for ever — and because `awaiting` short-circuits the verdict ladder in `cases.ts` it never became `stalled` either, so the one tab built to show what the queue hides showed nothing. *Silence is never consent* held only in the weak sense: no action was executed, and the escalation meant to replace it was the part that did not happen. **The general rule is about the TESTS, not the code**: every test of that sweep called it by hand, which is the one question a caller-less feature answers perfectly — a test that invokes the thing cannot tell you whether anything else does. This table already carries *grep for the other callers of the thing you fixed*; the inverse is *grep for the first one*, and the tell is infrastructure built for a caller nobody wrote. What closes it is a test that reads `server/api.ts` (it cannot be imported: importing it opens a port, which is why `app.ts` exists) with its COMMENTS STRIPPED, or the paragraph above the call goes on vouching for a call somebody removed. Two things the wiring taught. **A guard becomes live the day it is reached**: the sweep loop had no fault isolation, so a human answering one approval in the instant between `expiredWaits` listing it and `resolveWait` settling it threw out of the whole pass — measured — and every OTHER expired approval behind it was left waiting in silence; and the classification has to be READ FROM THE STORE (the wait gaining a `resumedAt`) rather than matched on the error text, with a flag saying whether THIS pass is the one that settled it — without that flag a failure after our own `resolveWait` re-reads the `resumedAt` we just wrote and files itself as a normal race, which the test caught and the reading did not. **And the mirror of a rule is sometimes the rule already applied.** The sweep's failures looked like the `pg`-empty-message trap, so the first fix ran them through `describePgError` a second time; booting the console against no database printed what actually arrives — *« Database (SELECT): connect ECONNREFUSED 127.0.0.1:5432 »* — because `PgRunStore.q()` is a single choke point that describes every query failure already. Describing it twice prints one cause under two prefixes. Found by running it |
| **A diagnostic that answered about a port it had not dialled** | `POST /api/settings/test/database` does `Number(body.port ?? d.port)` and hands it to `tcpProbe`. `saveConfig`, ten lines away in `config.ts`, already clamps the same value into `[1, 65535]` — **so the rule existed, on the screen an operator uses when things already work**, and the diagnostic they press when things do not had none. It answered two different wrong things, both measured through the real handler and then over real sockets. **The 200 is the worse one.** The port field is `type="number"`, so clearing it sends `Number('') === 0`; `connect()` came back `ECONNREFUSED` and the known-codes table words that *« Connection refused: nothing is listening on this port »* — a verdict about a machine, for a value that named no port, about a field SAVE would have turned into 5432. Node's own message for port 0 does not even carry a port (`connect ECONNREFUSED 127.0.0.1`, probed) because the OS never dialled one: *a sentence must not be reachable from a state it does not describe*, the `makeLlm` "answered with no content" family, on the button pressed to find out whether the database is the problem. **The 500 is a sender fault filed as a server fault**: `70000`, `-1`, `5432.5` and a non-numeric string make `createConnection` throw `RangeError ERR_SOCKET_BAD_PORT` **synchronously inside `tcpProbe`'s promise executor**, so nothing catches it, the route falls to `app.ts`'s last net and the operator reads V8's *« Received type number (70000) »* while the log gains a stack under `[menater] uncaught error`. Three things the fix had to get right. **It refuses, it does not clamp** — probing 65535 because somebody typed 70000 answers a question nobody asked, and dresses it as a diagnosis; a mutation carries that. **The two questions keep different status codes**: "is something listening" is about the network and stays a 200 with `ok: true`/`ok: false`, "that is not a port" is about the request and leaves as a 400 — so the guard is at the CALLER and deliberately NOT inside `tcpProbe`, which could only answer `ok: false` there, i.e. the network sentence again. **And a shape the interface cannot send is not coerced**: `Number(true)` is 1 and `Number('')` is 0, both legal ports nobody named. The test that closes it starts its own listener on a port the OS hands out rather than asserting some fixed port is closed — that is the `persistent-cache.test.ts` lesson, *a test must provoke the failure it is testing, never borrow one from the machine* |
| **A test that cannot run is not protecting anything, and the thing it guarded had already broken** | `store-contract.test.ts` exists for one reason: `RunStore` has two implementations, nothing naturally keeps them in step, and a divergence would only show in production, on the resume of an interrupted run — the worst possible moment. So it runs the SAME suite against both. Its Postgres half is gated on `MENATER_TEST_PG`, and the file says so in its own header, calling a silent never-running test *worse than an absent one*. **It had never been set.** The first time it ran — a real Postgres 16.13 stood up in the nightly container — it was red on unmodified `main`, and what it reported was exactly the drift it exists to catch: `MemoryRunStore.resolveWait` threw `jeton d'attente inconnu` where `PgRunStore` throws `unknown wait token`. Not an internal string: `routes/approvals.ts` puts `err.message` into `approvalFailed`, so an operator was answered in French or English depending on which store was mounted, in a product that went English-only. **The skip was honest and still cost the whole guarantee** — the gate was documented, deliberate and visible in every run's output as `skipped`, and none of that makes a comparison that never happens into a comparison. Two more things the run settled, and they are the general ones. **A premise expires**: three successive nights deferred work on `pg-store.ts` because *this environment has no database*, which was inherited rather than re-checked; it is false, and the cost was a whole subject left standing. And **a skipped test hides the PRs that leaned on it**: the four `openWaitOfRun` contract tests written with the approval fix were merged unverified against real SQL and happen to pass — nobody knew, which is not the same as their being right. The lesson is not *delete the gate*: it is that a conditional test is a claim about an environment, and nothing was checking that claim was ever met |
| **A rule kept by the API, and not by the file server ten lines away** | `respond.ts` gzips every JSON answer over 4 kB, under a comment explaining that the snapshot "runs to several hundred kB" and "compresses about tenfold". `static.ts` — same process, same socket, same deployment — handed out a **474 kB JavaScript bundle in the clear**, and the whole interface with it. Measured on a real socket against this repository's own build, counting bytes as they travelled: a cold load moved **629,637 bytes** where the same four files gzip to **175,581**, **3.6 times the bytes** — and in the normal deployment there is nothing in front of this process to do it instead, because *one application, one origin, one lock* is the reason there is no nginx. **Nothing fails, which is why it lasted**: it is a bill, and a bill shows up only if somebody goes looking. Sixth recurrence of *the mirror of a rule is not the rule*, and the nearest one yet — not another file, not the other half of the product, but the module next door, importing the very constant that states the threshold. Two things the fix had to get right, and each is measured rather than reasoned. **Streamed, never `gzipSync`**: the synchronous call is 12.4 ms of CPU and **15.3 ms of blocked event loop**, `createGzip()` is the same work on libuv's threadpool for a **2.1 ms** lag — and this is the thread that also answers the ingestion webhook, so WHERE the CPU lands matters more than how much of it there is. **And no memo, although `respond.ts` has one**: that memo exists because one snapshot object is answered many times a second, whereas every asset here carries a fingerprint and a year of `immutable`, so a browser fetches it once per deployment — a cache would save nothing and add an invalidation for somebody to get wrong. The boundaries are claimed and not assumed: the list says what to COMPRESS rather than what to skip (a woff2 is a compressed container, and gzipping it spends CPU for a handful of bytes), the floor is the JSON routes' own `GZIP_MIN_BYTES` imported rather than restated, and `Vary: Accept-Encoding` goes on exactly the answers that depend on that header — without it a cache may hand the gzipped variant to a client that asked for bytes it can read, and `immutable` would make that mistake a year long. **The tests had to leave the fake**: the response double in `static.test.ts` is enough to read a header back and can vouch for no body at all, because `pipe()` into something that is not a `Writable` moves nothing — a test driving it would agree with any implementation, working or not. Real sockets, the same lesson `fetchWithDeadline`'s redirect tests paid for. One more thing the change owed: `pipe()` forwards no error, so a read that fails after the head is written takes the process down — survivable while the chain was one stream long, ours to state the moment a second stream joined it |
| **Two MCP servers that pretty-printed an answer nobody asked to be pretty** | Seventh recurrence of *the mirror of a rule is not the rule*, and this time it is a bill rather than a failure. The assistant panel serialises a tool result compactly — `chat.ts`'s `capResult` measures and cuts on `JSON.stringify(result)` with no spacing, and `providers.ts` sends it that way to all four providers. `assistant/mcp.ts`, the SAME fourteen getters one file over, wrote `JSON.stringify(result, null, 2)` into `content[0].text` and into every `resources/read`. **That block is what the other side's MODEL reads**: `structuredContent` beside it is for the client's code, the text is what lands in a context window, and two-space indentation is a line break plus an indent run per key that says nothing the JSON does not. Measured by driving the real endpoint over this installation's sample window: **24,219 B → 19,473 B over the fourteen tools (−19.6%)** and 26,120 → 21,177 over the nine resources (−18.9%), stable at 17–25% per call. The same line in `VulnPipe/src/mcp-server/server.ts` is worse and cheaper to see: **10,684 → 7,940 B (−25.7%)**, −29.6% on a context bundle at the 24 kB budget, and its only client (`nodes/shared/mcp-client.ts`) reads `structuredContent` and never touches `content` — so the indentation was serialised, pushed over stdio and thrown away. **The headline number this was carried under was measuring something else**, which is the general lesson: two nightly entries recorded the survey figure *« 11,191 B where 5,123 B would do »* as the biggest token saving on the surface. Re-measured, 11,191 is the text block PLUS `structuredContent` and 5,123 is `structuredContent` alone — i.e. the number priced DELETING the text block, which the spec asks for and which a client that parses `content[0].text` is entitled to. The saving that actually exists is the indentation, a fifth of it. **A survey that is not re-measured turns into a number people plan around**, and the one thing it cannot tell you is what it measured. What the fix does NOT do: it does not drop the duplicate, and it does not touch `decision.ts`'s alert block, which pretty-prints the same way into the triage prompt — measured at 423 B per alert, 5.6% once a 6 kB raw log is in it, and changing what a model is shown cannot be A/B'd from an environment with no key |
| **A page that hid its own right-hand edge, measured everywhere except on the product** | Ninth recurrence of *the mirror of a rule is not the rule*, and the sharpest one yet: the rule was not merely written elsewhere, it was **tooled**. `site/check.cjs` drives Chromium over the marketing site at 320, 390, 768 and 1400 px and reports horizontal overflow, under a header saying *« a page that scrolls sideways on a phone hides its own right-hand edge »*. Two static brochure pages were swept on every build; the CONSOLE was swept by nobody. Measured on the built bundle, every tab and every sub-tab: at **320 px** — the width WCAG 2.2 § 1.4.10 (Reflow, AA) names, and what a 375 px phone becomes at 125 % zoom — the Ingestion tab's document was **330 px**, Health **529** and Settings → MCP **546**; at 375 px, Health and MCP the same. **Three causes, and the first two each teach a rule.** (1) The diagnostic card prints `postgresql://user:***@host:5432/db`, one 51-character token with nowhere to break, and a grid item has `min-width: auto` — so that token became the item's automatic minimum and FLOORED the track at **503.6 px inside a 325 px grid**. `styles.css` already carried `@media (max-width: 760px) { .soc-health-grid { grid-template-columns: 1fr } }`, written for exactly this, and it cannot work: it chooses how many tracks there are, not how narrow one may become. Measured, each remedy alone: `min-width: 0` → still 512 px, `minmax(min(260px, 100%), 1fr)` → still 512 px, letting the value break → 375. **And the spelling of the break is not interchangeable**: `overflow-wrap: anywhere` and `word-break: break-all` take part in min-content sizing and fix it; `overflow-wrap: break-word` wraps the visible line, leaves the minimum where it was and measured **529 px unchanged** — a fix that looks applied and is not. (2) `.soc-lang-wide` is the segmented picker SHARED by six screens (a rule's action and severity, the MCP client, the ingestion mode, the setup wizard, the fast lane): an `inline-flex` that never wrapped is as wide as the sum of its buttons, so the longest labels take the page with them. Two of the six had already failed; `flex-wrap: wrap` fixes all six, where shortening the label you noticed fixes one. (3) A 43-character file path (`%APPDATA%\\Claude\\claude_desktop_config.json`) inside ordinary prose: 302 px of min-content in 270 px of room. It **passed at 375 px and failed at 320** — a width that passes is not a width that has margin, which is why the sweep is run at the criterion's width and not at the phone you happen to own. What stays open is the instrument: the suite cannot see any of this, because jsdom computes no layout and `scrollWidth` is 0 there, so the guard is three assertions over `styles.css` paired with three over the rendered markup — a rule that exists is not a rule that reaches anything, and the 760 px override is what an orphaned rule looks like |
| **A role kept on the widget somebody measured, and on neither of the other two** | `role="tab"` does not describe a button, it ANNOUNCES a contract — the group is ONE stop in the tab order, the arrows choose inside it — and the pass that made `SectionTabs` keep it swept its own four call sites and stopped there. Two other composite widgets, **one per half of the product**, declared a composite role and implemented none of its keyboard: the theme grid (`role="radiogroup"`, six `role="radio"`, Settings → Console) and the scan launcher's target bar (`role="tablist"`, three `role="tab"`, the Code tab). Measured in Chromium on the built console: **6 tab stops of 6 and 6 Tab presses to cross the theme grid, 3 of 3 on the target bar, and every arrow key — Left, Right, Up, Down, Home, End — did nothing on either**. So a screen reader said « radio button, 1 of 6 » and « tab, 1 of 3 », and the key it had just announced was dead: a key you have been told works and that does not work does not read as a missing feature, it reads as a broken page. Tenth recurrence of *the mirror of a rule is not the rule*, and the shape of the miss is the transferable part: the earlier fix was written as a fix to a COMPONENT, so the grep that finds the others is not « who calls `SectionTabs` » but « what else wears a composite role » — `grep -o 'role="[a-z]*"'` over the product prints the whole inventory in one line, and it is what named these two. Two more things it taught. **The vertical arrows are an argument, never a default**: a horizontal tab bar leaves Up, Down and the page keys to the browser — the console's bar is sticky above content that scrolls — while a radio group answers to all four because a native `<input type="radio">` group does, so ignoring them under `role="radio"` would be a second, quieter lie; the shared helper takes an axis and the tests claim both directions. **And Chromium's AX tree reports `focusable=true` on a `tabIndex="-1"` member**, which is true and is not the question — programmatically focusable is not in the tab order, so what a keyboard user pays is measured by counting Tab presses out of the group, not by reading a property off the tree. The boundary that keeps the fix from becoming noise: six segmented pickers here wear `role="group"` with `aria-pressed`, which promises no keyboard at all, so every one of their buttons is legitimately its own stop |
...
What remains: the two flows do not talk *automatically*, and that is a choice
rather than a gap. **A scan finding can now become a case (J0.1)** — the report
carries a *Send to the triage queue* button per finding, and what it sends
enters `01-Ingestion` at the same line a Wazuh alert does. Nothing sends itself:
a human presses it. No alert starts a scan on its own (J0.2 is open).
...
**And the flaw itself can now enter the queue (J0.1).** `server/findings.ts`
maps one `ReportFinding` to the canonical alert shape,
`POST /api/findings/promote` hands it to `injectAlert`, and it runs
`01-Ingestion` — one pipeline, one set of guardrails, one audit chain, the same
rule a polled alert obeys. Four decisions hold it up:

| Decision | Why the other answer was worse |
|---|---|
| **A human presses a button** | A scan that raised its own alerts would spend a model call, and downstream an approval request, on a report nobody had read. Same rule as J0.3: it resolves, it never acts |
| **The alert carries NO host** | `isolate_host_temporary` is proposed from `dest_ip ?? host`. A finding says a route is exploitable and nothing about a machine being compromised; leaving `host` absent — which is also simply true — makes `isolationTarget()` refuse, so the contract's own safety property does the work with no second rule to keep in step |
| **Every repository- or model-authored string lands in a FENCED field** | `rule_name`, `raw_log`, `file_path`, `extensions`. You scan code you did not write, so a finding is attacker-influenceable text. `alert_id` is the field that proves the rule: it is not fenced, so it is DERIVED (a hash), never composed from the finding's own strings |
| **The scan run is part of the alert's identity** | See the traps table. Keyed on the flaw alone, a finding fixed in January and regressed in June could never be triaged again |

**The case now names the code it came from.** `scanRepository` in
`server/findings.ts` reads the scanned target back out of `extensions` — in the
file that writes it — and `snapshot.ts` asks it before the inventory, so a case
opened from a scan carries a `repository` the inventory could never have
resolved (it is keyed on machines, and this alert has none). The card shows the
target and its *Analyse this code* button opens the Code tab on it; `service`
is `null` there, because nobody has told us what the service is called and
repeating the target would dress one fact as two.

**What J0.1 still does not do:** nothing walks the other way — J0.2, an alert
offering a scan, is untouched.
...
(`server/assistant/tools.ts`) is closed, read-only, and holds **fourteen
getters** — the queue and one alert (`list_alerts`, `search_alerts`,
`get_alert`, `find_similar`), what ran and why (`get_trace`, `get_timeline`,
`explain_verdict`), what needs a human (`get_attention`, `get_health`,
`get_metrics`), rules and configuration (`list_rules`, `test_rule`,
`get_setup_state`), and the glossary (`explain_term`). There is no tool that
approves, isolates, closes, replays, edits a rule or writes a setting — the
third closed catalogue in this product, and the strictest. It is the SAME
fourteen the MCP endpoint exposes; there is no second list to audit.
...
| Layer | What it does |
|---|---|
| **No writes in the catalogue** | An injected instruction arrives at a model holding fourteen getters and no verb among them. This is the one that matters |
...
| **The prompt names every tool, one by one** | Its "what you can do" block described the catalogue in prose, written when there were seven. `TOOLS` reached fourteen and the prose did not move, so half of them — search, similar alerts, the timeline, the verdict story, a rule dry run, the attention roll-up and `explain_term` — were sent as schemas and mentioned nowhere in the instructions that say what to reach for. A prose summary cannot be checked; a list of names can, and `assistant.test.ts` now fails in both directions |
...
| **The outbound credential is a NAME from the closed catalogue, and the address stays the one that was typed** | `isManagedCredential` gates the name. Reading an arbitrary environment variable into a header, at an address typed into the same form, is an exfiltration primitive with a save button. The address is `http(s)` only, for the same reason the Lookup tab dials nothing it was handed — and it is checked at save time, on the string an operator typed, which left the other half open: `fetch` follows redirects, so the source could nominate the host its own credential was delivered to. `fetchWithDeadline` follows a redirect only while it stays on that host; see the traps table |
...
session history lives in the browser tab and dies with it. No alert starts a
scan on its own — that is J0.2, and this tab deliberately did not invent a
second path into the pipeline. (A scan finding *can* now open a case, by a
human pressing a button on it: J0.1, `server/findings.ts`.) What it does do is
...
| **The grid is a radio group, and it behaves like one** | `role="radiogroup"` promises one stop in the tab order and arrows that choose inside it. It kept none of that until C0.32: six stops, six Tab presses to cross, every arrow dead. The chosen theme carries the stop now, and an arrow applies the next palette — which is what a click here already does. See the traps table |
...
npm run typecheck
npm run test           # 1219 tests, 1 skipped (the one needing a database)
npm run build
