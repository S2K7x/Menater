...
**What J0.1 does not do:** the case it opens carries the scanned target in
`extensions`, but `snapshot.ts` resolves `repository` from the inventory by
host — and this alert has no host — so the card does not name the code it came
from. The next slice is that back-reference, and J0.2 (an alert offering a scan)
is untouched.
