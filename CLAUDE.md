...
| **Three parties, two already agreed, and the one in the middle invented a vocabulary** | The browser posts `{ decision, approver, reason }` to `/api/approvals/:token/resume` (`src/lib/api.ts`) and `interpretApproval` reads `payload.decision`, `payload.approver`, `payload.reason`. **Both ends of the conversation were right**; `routes/approvals.ts`, sitting between them, translated into a third form — `{ approved, human_reasoning, approver: {…} }` — that nothing on the far side read. Nothing failed: `p.decision` was `undefined`, so the transform's own rule (*anything that is not exactly `approve` or `reject` is silence*) turned a human's explicit yes into `timeout_escalated`. Measured through the real route over the real pipeline: the approved containment was **never dialled**, and the audit row — append-only, immutable, the product's only tamper-evidence — recorded `routing_outcome: "rejected"`, `"reason": "Action refused by a human."` about a yes. It fails safe on the ACTION and lies in the RECORD, in the one place that cannot be corrected afterwards. Three general lessons. **A mismatch between two correct endpoints is invisible from either one**, and from every unit test of either one — only a test that drives the whole boundary sees it, which is why the file that closed this drives the REAL route and reads the ENGINE's conclusion out of the run journal rather than the sentence the route printed. **A decision is a WORD, never a boolean**: `approve`, `reject` and *nobody answered* are three facts, and the moment the route reduced them to `approved: boolean` the third one had nowhere to live — so an unrecognised answer is refused at the route with a named 400, because forwarding it spends the token (`resolveWait` is irreversible) and files a silence that never happened. **And the transport sends a CLAIM, never a RECORD**: the route was building the approver record itself, `identity_source` included, on the side of the boundary a payload can reach — a transport able to author `signature_verified: true` writes an authentication that never happened into the hash chain. The guardrail mints it now, and the value is `console_self_declared`, which this table and `dashboard/README.md` had documented for so long that nobody noticed **nothing produced it**: the route built it and `interpretApproval` threw it away |
| **A screen that answers with an identifier nobody ever gave it** | The other half of the row above, and it had been true for the whole life of the feature: **the console could not answer an approval at all.** `CaseView` computes `canSubmit = approver && Boolean(execId) && !busy` from `approval.execution_id`, and `cases.ts` never wrote that field — so both buttons rendered `disabled` on every real case, and said nothing, because the other term of that `&&` is the operator's own name: a greyed-out button is indistinguishable from *you have not finished typing*. Only `demo.ts` set it, so the screen looked right in demonstration mode and only there. **The half underneath is the one to keep.** Posting the identifier the card DOES hold — the routing run — answered **404 « That approval is not open any more — it was already answered, or it timed out. The first answer stands; nothing was lost. »** over a wait that was wide open and a run sitting in `waiting`. Every clause of that sentence is false in that state, and it is reassuring: it tells an operator nothing was lost, on the control that carries « no irreversible action without explicit human approval ». Same rule as `llm` answering *« the model answered with no content »* about a model it never asked — **a sentence must not be reachable from a state it does not describe** — pointed for once at the person the product exists for. **The decision it looked like was not the decision it was.** It reads as *token or run id?*, i.e. which spelling wins; it is really *what is already published?* — and the answer was measured, not argued: the wait token is written NOWHERE outside the database (`resumeUrl` builds `/?run=<run id>`, the snapshot carries run ids on every stage of every case), so its only holder is the store, while the run is the handle Slack and the snapshot already publish. Accepting the run grants a signed-in tab nothing it did not have; shipping the token to every open tab would have MINTED a capability that does not exist today. `openWaitOfRun` returns only an OPEN wait, which is what finally makes that 404 true when it is shown, and `resumeWait` resolves `wait.token` and never its own argument. **And the reason nobody had fixed it was a premise that had stopped holding**: the previous night filed it as undecidable because the Postgres half « cannot be verified without a database and this environment has none ». Postgres 16.13 installs in the nightly container, so `store-contract.test.ts` — written to run the SAME suite against both implementations, and skipped for want of `MENATER_TEST_PG` — ran for the first time and immediately reported the drift it exists to catch: `MemoryRunStore` threw `jeton d'attente inconnu` where `PgRunStore` throws `unknown wait token`, a sentence that reaches an operator through `approvalFailed(err.message)`. **A test nobody can run is a test that is not protecting anything**, and the thing it was not protecting had already broken. Check the premise before inheriting it |
